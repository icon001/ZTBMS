...
type
  TArmAreaRelayModeState = class(TComponent)
  private
    arrArmAreaMode : array[0..576] of TWatchMode;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  public
    procedure SetArmAreaMode(aNo:integer;aWatchMode:TWatchMode);
    function GetArmAreaMode(aNo:integer):TWatchMode;
  end;
  ...
  public
    ...
    function GetEcuHEXState(aEcuID:integer):string;
...
begin
  ...
  else if stWatchMode = '0110' then msDeviceState.WatchMode := cmJaejung
  ...
  else if stDoorManager1 = '100' then msDeviceState.DoorManageMode1 := dmMaster
  ...
  else if stDoorManager2 = '100' then msDeviceState.DoorManageMode2 := dmMaster
...
function TNodeLockState.GetEcuHEXState(aEcuID: integer): string;
var
  stBinary : string;
  stHex : string;
  nIndex : integer;
...
procedure TDeviceMonitorState.SetDoorState1(const Value: TDoorDSState);
begin
  FDoorState1 := Value;
end;

procedure TDeviceMonitorState.SetDoorState2(const Value: TDoorDSState);
begin
  FDoorState2 := Value;
end;
...
constructor TArmAreaRelayModeState.Create(AOwner: TComponent);
var
  i : integer;
begin
  inherited;
  for i := Low(arrArmAreaMode) to High(arrArmAreaMode) do
  begin
    arrArmAreaMode[i] := cmNothing;
  end; 
end;

destructor TArmAreaRelayModeState.Destroy;
begin

  inherited;
end;

function TArmAreaRelayModeState.GetArmAreaMode(aNo: integer): TWatchMode;
begin
  result := cmNothing;
  if aNo > High(arrArmAreaMode) then Exit;
  result := arrArmAreaMode[aNo];
end;

procedure TArmAreaRelayModeState.SetArmAreaMode(aNo: integer;
  aWatchMode: TWatchMode);
begin
  if aNo > High(arrArmAreaMode) then Exit;
  arrArmAreaMode[aNo] := aWatchMode;
end;
