...
type
  ...
  public
    ...
    Function DeleteTB_EMPLOYEE(aEmpID,aCompanyCode,aCardNo,aFormName:string):Boolean;
    ...
    function InsertIntoTB_HURELAY_Value(aID,aName,aLocCode,aStatus,aVacancy:string):Boolean;
    Function InsertIntoTB_LOGINCREATEHIS(auserid,ausername,auserPw,aGradeCode,aMaster,aCompanyCode,aJijumCode,aDepartCode,aBuildingGrade,aDongCode,aFloorCode,aAreaCode,aMode:string):Boolean;
    ...
    Function UpdateTB_DEVICECARDNO_AllMemory(aState:String):Boolean;
    ...
    function UpdateTB_LOCATION_Field_StringValue(aDONGCODE,aFLOORCODE,aAREACODE,aFieldName,aValue:string):Boolean;
  ...
  public
    ...
    Function GetTB_LOGINCREATEHIS_Seq(aData:string):string;
...
function TdmDBFunction.CheckTB_CARDFromEmployeeID(aCompanyCode,
...
function TdmDBFunction.DeleteTB_CARD(aCardNo: string): Boolean;
var
  stSql : string;
  bResult : Boolean;
begin
...
function TdmDBFunction.DeleteTB_EMPLOYEE(aEmpID, aCompanyCode,
  aCardNo,aFormName: string): Boolean;
var
  stSql : string;
  stFdmsId : string;
begin
  ...
  stSql := CommonSql.InsertIntoTB_EMPLOYEECHANGE(aCompanyCode,aEmpID,aCardNo,aFormName,'3');
...
function TdmDBFunction.GetEmployeeFdmsID(aCompanyCode,
...
function TdmDBFunction.GetTB_LOGINCREATEHIS_Seq(aData: string): string;
var
  stSql : string;
  TempAdoQuery :TADOQuery;
begin
  ...
  stSql := 'select MAX(LC_SEQ) as LC_SEQ from TB_LOGINCREATEHIS ';
  stSql := stSql + ' Where  GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND LC_DATE = ''' + aData + ''' ';
  ...
  Try
    ...
    begin
      ...
      End;
      if recordCount < 1 then Exit;
      result := inttostr(FindField('LC_SEQ').AsInteger + 1);
...
function TdmDBFunction.InsertIntoTB_HURELAY_Value(aID, aName, aLocCode,
  aStatus, aVacancy: string): Boolean;
var
  stSql : string;
begin
  stSql := ' Insert Into TB_HURELAY( ';
  stSql := stSql + ' HR_ID,';
  stSql := stSql + ' HR_NAME,';
  stSql := stSql + ' HR_LOCCODE,';
  stSql := stSql + ' HR_STATUS,';
  stSql := stSql + ' HR_VACANCY )';
  stSql := stSql + ' values( ';
  stSql := stSql + '''' + aID + ''',';
  stSql := stSql + '''' + aName + ''',';
  stSql := stSql + '''' + aLocCode + ''',';
  stSql := stSql + '''' + aStatus + ''',';
  stSql := stSql + '''' + aVacancy + '''';
  stSql := stSql + ')';
  result := DataModule1.ProcessExecSQL(stSql);
end;

function TdmDBFunction.InsertIntoTB_LOGINCREATEHIS(auserid, ausername,
  auserPw, aGradeCode, aMaster, aCompanyCode, aJijumCode, aDepartCode,
  aBuildingGrade, aDongCode, aFloorCode, aAreaCode,aMode: string): Boolean;
var
  stSql : string;
begin
  result := False;
  stSql := ' Insert Into TB_LOGINCREATEHIS( ';
  stSql := stSql + ' GROUP_CODE,';
  stSql := stSql + ' LC_DATE,';
  stSql := stSql + ' LC_SEQ,';
  ...
  stSql := stSql + ' LO_AREACODE,' ;
	stSql := stSql + ' LC_MODE,';   //1.입력,2.수정,3.삭제
  stSql := stSql + ' LC_PCIP) ';
  stSql := stSql + ' values( ';
  stSql := stSql + '''' + GROUPCODE + ''',';
  stSql := stSql + '''' + FormatDateTime('yyyymmdd',now) + ''', ';
  stSql := stSql + '' + GetTB_LOGINCREATEHIS_Seq(FormatDateTime('yyyymmdd',now)) + ',';
  stSql := stSql + '''' + auserid + ''',';
  stSql := stSql + '''' + auserPw + ''',';
...
function TdmDBFunction.UpdateTB_DEVICECARDNO(aCardNo,
...
function TdmDBFunction.UpdateTB_DEVICECARDNO_AllMemory(
  aState: String): Boolean;
var
  stSql : string;
begin
  Result := False;
  stSql := 'Update TB_DEVICECARDNO Set DE_MEMLOAD = ''' + aState + ''' ';     //송신중으로 변경
...
function TdmDBFunction.UpdateTB_LOCATION_Field_StringValue(aDONGCODE,
  aFLOORCODE, aAREACODE, aFieldName, aValue: string): Boolean;
var
  stSql : string;
begin
  ...
  stSql := stSql + ' WHERE GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND LO_DONGCODE = ''' + aDONGCODE + ''' ' ;
