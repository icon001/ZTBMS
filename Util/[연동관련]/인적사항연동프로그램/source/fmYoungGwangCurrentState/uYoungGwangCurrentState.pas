...
type
  TfmYoungGwangCurrentState = class(TfmASubForm)
    AdvPanel2: TAdvPanel;
    Panel2: TPanel;
  ...
  public
    ...
    procedure EventState(aGubun,aJijumCode,aEmCode,aEmCophone,aEmName,aDepartName,
                               aCardSeq,aRegState,aEmState,aUpdateTime,aResult:string);
  end;

var
  fmYoungGwangCurrentState: TfmYoungGwangCurrentState;
...
procedure TfmYoungGwangCurrentState.FormCreate(Sender: TObject);
begin
  Self.ModuleID := 'YOUNGGWANGSTATE';

  DisplayList := TStringList.Create;

end;

procedure TfmYoungGwangCurrentState.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  L_bClose := True;
  SearchTimer.Enabled := False;
  if G_bApplicationTerminate then Exit;
  self.FindSubForm('Main').FindCommand('YoungGwangState').Params.Values['VALUE'] := 'FALSE';
  self.FindSubForm('Main').FindCommand('YoungGwangState').Execute;
  Action := caFree;
  DisplayList.Clear;
  DisplayList.Free;
end;

procedure TfmYoungGwangCurrentState.FormShow(Sender: TObject);
begin

  self.FindSubForm('Main').FindCommand('YoungGwangState').Params.Values['VALUE'] := 'TRUE';
  self.FindSubForm('Main').FindCommand('YoungGwangState').Execute;

end;

procedure TfmYoungGwangCurrentState.CommandArrayCommandsTCommand1Execute(
  Command: TCommand; Params: TStringList);
begin
  Close;
end;

procedure TfmYoungGwangCurrentState.CloseForm;
begin
  Close;
end;

procedure TfmYoungGwangCurrentState.BatchDisplay(aData:string);
var
  nPos : integer;
begin
...
procedure TfmYoungGwangCurrentState.btn_CloseClick(Sender: TObject);
begin
  inherited;
  Close;
end;
...
procedure TfmYoungGwangCurrentState.RelayAdoEvent(aSql: string);
begin
end;

procedure TfmYoungGwangCurrentState.EventState(aGubun, aJijumCode,
...
procedure TfmYoungGwangCurrentState.SearchTimerTimer(Sender: TObject);
begin
  inherited;
  SearchTimer.Enabled := False;
...
initialization
  RegisterClass(TfmYoungGwangCurrentState);
Finalization
  UnRegisterClass(TfmYoungGwangCurrentState);
