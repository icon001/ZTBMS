...
uses
  ...
  StdCtrls, DB, ADODB, Gauges,ActiveX;
...
begin
  ...
  if(stBuildingCode<> '000') then stSql := stSql + ' AND a.LO_DONGCODE = ''' +  stBuildingCode + ''' ';
  if(stFloorCode<> '000') then stSql := stSql + ' AND a.LO_FLOORCODE = ''' +  stFloorCode + ''' ';
  if(stAreaCode<> '000') then stSql := stSql + ' AND a.LO_AREACODE = ''' +  stAreaCode + ''' ';
...
procedure TfmGroupGrade.LoadBuildingCode(cmb_Box: TComboBox);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
begin
...
procedure TfmGroupGrade.LoadFloorCode(aBuildingCode: string;
  cmb_Box: TComboBox);
var
  stSql : string;
  TempAdoQuery:TADOQuery;
...
procedure TfmGroupGrade.LoadAreaCode(aBuildingCode, aFloorCode: string;
  cmb_Box: TComboBox);
var
  stSql : string;
  TempAdoQuery : TADOQuery;
...
procedure TfmGroupGrade.cmb_BuildingCodeChange(Sender: TObject);
var
  stBuildingCode : string;
  stFloorCode : string;
  stAreaCode : string;
...
procedure TfmGroupGrade.cmb_FloorCodeChange(Sender: TObject);
var
  stBuildingCode : string;
  stFloorCode : string;
  stAreaCode : string;
...
procedure TfmGroupGrade.cmb_AreaCodeChange(Sender: TObject);
begin
  cmb_GroupCodeChange(cmb_GroupCode);
end;
