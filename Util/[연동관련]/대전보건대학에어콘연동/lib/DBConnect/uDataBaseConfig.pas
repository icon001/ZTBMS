...
type
  ...
  private
    ...
    Function CreateTB_SYSAIRCONDMS:string;
    Function CreateTB_SYSAIRCONCONTROLER:string;
    Function CreateTB_SYSAIRCONRELAY:string;
    Function CreateTB_SYSAIRCONDEVICE:string;
...
function TDataBaseConfig.CreateTB_SYSAIRCONCONTROLER: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SYSAIRCONCONTROLER (';
  stSql := stSql + ' SD_NODENO integer NOT NULL,';
  stSql := stSql + ' SD_CONTROLERID varchar(2) NOT NULL,';
  stSql := stSql + ' SD_CONTROLERNAME varchar(30) NULL,';
  stSql := stSql + ' PRIMARY KEY (SD_NODENO,SD_CONTROLERID) ';
  stSql := stSql + ' ) ';
...
function TDataBaseConfig.CreateTB_SYSAIRCONDEVICE: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SYSAIRCONDEVICE (';
  stSql := stSql + ' SD_NODENO integer NOT NULL,';
  stSql := stSql + ' SD_CONTROLERID varchar(2) NOT NULL,';
  stSql := stSql + ' SD_RELAYID varchar(2) NOT NULL,';
  stSql := stSql + ' SD_DEVICEID varchar(2) NOT NULL,';
  stSql := stSql + ' SD_DEVICENAME varchar(30) ,';
  stSql := stSql + ' SD_DEVICESTATE varchar(10) ,';
  stSql := stSql + ' AC_NODENO Integer ,';
  stSql := stSql + ' AC_ECUID varchar(2) ,';
  stSql := stSql + ' SD_RELAYSTATE char(1) ,';
  stSql := stSql + ' PRIMARY KEY (SD_NODENO,SD_CONTROLERID,SD_RELAYID,SD_DEVICEID) ';
  stSql := stSql + ' ) ';
...
function TDataBaseConfig.CreateTB_SYSAIRCONDMS: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SYSAIRCONDMS (';
  stSql := stSql + ' SD_NODENO integer NOT NULL,';
  stSql := stSql + ' SD_DMSNAME varchar(30) NULL,';
  stSql := stSql + ' SD_IP varchar(30) NULL,';
  stSql := stSql + ' SD_PORT varchar(30) NULL,';
  stSql := stSql + ' PRIMARY KEY (SD_NODENO) ';
  stSql := stSql + ' ) ';

  result := stSql;
end;

function TDataBaseConfig.CreateTB_SYSAIRCONRELAY: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SYSAIRCONRELAY (';
  stSql := stSql + ' SD_NODENO integer NOT NULL,';
  stSql := stSql + ' SD_CONTROLERID varchar(2) NOT NULL,';
  stSql := stSql + ' SD_RELAYID varchar(2) NOT NULL,';
  stSql := stSql + ' SD_RELAYNAME varchar(30) NULL,';
  stSql := stSql + ' PRIMARY KEY (SD_NODENO,SD_CONTROLERID,SD_RELAYID) ';
  stSql := stSql + ' ) ';
...
begin
  ...
  stSql := stSql + ' where CO_CONFIGGROUP = ''SYSAIRCON'' ';
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('SYSAIRCON','TABLE_VER','1','시스템에어콘테이블');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('SYSAIRCON','RELAYTIME','30','연동시간');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := CreateTB_SYSAIRCONDMS;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := CreateTB_SYSAIRCONCONTROLER;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := CreateTB_SYSAIRCONRELAY;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := CreateTB_SYSAIRCONDEVICE;
  dmDBModule.ProcessExecSQL(stSql);
