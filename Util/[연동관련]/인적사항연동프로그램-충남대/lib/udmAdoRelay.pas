...
type
  ...
  public
    ...
    Function AdoRelayConnected(aDataBaseType,aServerIP,aServerPort,aUserID,aUserPw,aDataBaseName:string;aMsgShow:Boolean=False):Boolean;
  public
...
function TdmAdoRelay.AdoRelayConnected(aDataBaseType, aServerIP,
  aServerPort, aUserID, aUserPw, aDataBaseName: string;aMsgShow:Boolean=False): Boolean;
var
  conStr : string;
begin
  result := False;
  ...
  end else if aDataBaseType = '5' then  //Cubrid
  begin
(*    conStr := 'Provider=CUBRIDProvider;';
    conStr := constr + 'Location ='+ aServerIP + ';';
    conStr := constr + 'Port ='+ aServerPort + ';';
    conStr := constr + 'Password=' + aUserPw + ';';
    conStr := constr + 'User ID='+ aUserID + ';';
    conStr := constr + 'Data Source=' + aDataBaseName + ';';
    conStr := constr + 'Fetch Size = 100;';
    conStr := constr + 'Persist Security Info=True';
*)
    conStr := 'driver={CUBRID Driver};';
    conStr := constr + 'server=' + aServerIP + ';';
    conStr := constr + 'port='+ aServerPort + ';';
    conStr := constr + 'db_name=' + aDataBaseName + ';';
    conStr := constr + 'User ID='+ aUserID + ';';
    conStr := constr + 'Password=' + aUserPw +';';
//    conStr := 'driver={CUBRID Driver};server=203.249.39.19;port=33000;db_name=ISSUE;User ID=dba;Password=inzenst;';
//    conStr := 'Provider=CUBRIDProvider;Location=203.249.39.19;Port=33000;Data Source=ISSUE;User ID=dba;Password=inzenst;Cache Authentication=False;Encrypt Password=False;Mask Password=False;Persist Encrypted=False;Persist Security Info=True';
  end else if aDataBaseType = '6' then  //Oracle
  begin
    //conStr := 'Provider=OraOLEDB.Oracle;';
    //conStr := constr + 'Data Source=(DESCRIPTION=(CID=GTU_APP)(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=' + aServerIP + ')(PORT=' + aServerPort + ')))(CONNECT_DATA=(SID=' + aDataBaseName + ')(SERVER=DEDICATED)));';
    conStr := 'Provider=OraOLEDB.Oracle;';
