...
type
  ...
  private
    ...
    Function AlterTB_ACCESSEVENT_ACRelayAdd:string;
    Function AlterTB_DOOR_YRDOORNO:string;
    Function CreateTB_YSReserveHis : string;
    Function CreateTB_YSReserveDOOR : string;
...
function TDataBaseConfig.AlterTB_ACCESSEVENT_ACRelayAdd: string;
var
  stSql : string;
begin
  ...
  begin
    stSql := 'ALTER TABLE TB_ACCESSEVENT alter column  AC_RELAY text(1) NULL ';
  end else if UpperCase(G_stDBType) = 'FB' then
  begin
    stSql := 'ALTER TABLE TB_ACCESSEVENT alter column AC_RELAY char(1) NULL ';
  end;
  result := stSql;

end;

function TDataBaseConfig.AlterTB_DOOR_YRDOORNO: string;
var
  stSql : string;
begin
  if UpperCase(G_stDBType) = 'MSSQL' then
  begin
    stSql := 'ALTER TABLE TB_DOOR ADD YR_DOORNO varchar(10) NULL ';
  end else if UpperCase(G_stDBType) = 'PG' then
  begin
    stSql := 'ALTER TABLE TB_DOOR ADD YR_DOORNO varchar(10) NULL ';
  end else if UpperCase(G_stDBType) = 'MDB' then
  begin
    stSql := 'ALTER TABLE TB_DOOR alter column  YR_DOORNO text(10) NULL ';
  end else if UpperCase(G_stDBType) = 'FB' then
  begin
    stSql := 'ALTER TABLE TB_DOOR alter column YR_DOORNO varchar(10) NULL ';
  end;
  result := stSql;

end;

function TDataBaseConfig.CreateTB_YSReserveDOOR: string;
var
  stSql : string;
begin
  if UpperCase(G_stDBType) = 'MSSQL' then
  begin
    stSql := 'Create Table TB_YSReserveDOOR (';
    stSql := stSql + ' YR_KEY integer NOT NULL,';        //키
    stSql := stSql + ' YR_DOORNO varchar(10) NOT NULL,';       //출입문아이디
    stSql := stSql + ' YR_REGGUBUN char(1) NOT NULL,'; //등록구분.,0.삭제,1.등록
    stSql := stSql + ' CA_CARDNO varchar(30) NOT NULL,';    //카드번호
    stSql := stSql + ' YR_TIME varchar(17),';    //시간
    stSql := stSql + ' YR_WORKTYPE char(1),'; //작업구분  'N'.미작업,'Y'.작업완료
    stSql := stSql + ' YR_UpdateTime varchar(17),';    //작업시간
    stSql := stSql + ' PRIMARY KEY (YR_KEY,YR_DOORNO,YR_REGGUBUN,CA_CARDNO) ';
    stSql := stSql + ' ) ';
  end else if UpperCase(G_stDBType) = 'PG' then
  begin
    stSql := 'Create Table TB_YSReserveDOOR (';
...
function TDataBaseConfig.CreateTB_YSReserveHis: string;
var
  stSql : string;
begin
  if UpperCase(G_stDBType) = 'MSSQL' then
  begin
    stSql := 'Create Table TB_YSReserveHis (';
    stSql := stSql + ' SEQ int IDENTITY NOT NULL,'; //순서
    stSql := stSql + ' YR_KEY integer,';        //키
    stSql := stSql + ' EM_CODE varchar(50),'; //사번
    stSql := stSql + ' EM_NAME varchar(100),';     //이름
    stSql := stSql + ' YR_DOORNO varchar(10),';       //출입문아이디
    stSql := stSql + ' YR_STARTTIME varchar(17),';    //시작시간
    stSql := stSql + ' YR_ENDTIME varchar(17),';      //종료시간
    stSql := stSql + ' CA_CARDNO varchar(30),';    //카드번호
    stSql := stSql + ' IS_USED varchar(2),';    //사용유무
    stSql := stSql + ' CO_JIJUMNAME varchar(100),';      //단과대명
    stSql := stSql + ' CO_DEPARTNAME varchar(100),';  //학과명
    stSql := stSql + ' YR_InsertTime varchar(17),';    //외출조작버튼
    stSql := stSql + ' PRIMARY KEY (SEQ) ';
    stSql := stSql + ' ) ';
  end else if UpperCase(G_stDBType) = 'PG' then
  begin
    stSql := 'Create Table TB_YSReserveHis (';
    stSql := stSql + ' SEQ serial NOT NULL,'; //순서
    stSql := stSql + ' YR_KEY integer,';        //키
    stSql := stSql + ' EM_CODE varchar(50),'; //사번
    stSql := stSql + ' EM_NAME varchar(100),';     //이름
    stSql := stSql + ' YR_DOORNO varchar(10),';       //출입문아이디
  ...
  begin
    stSql := 'Create Table TB_YSReserveHis (';
    stSql := stSql + ' SEQ COUNTER NOT NULL,'; //순서
    stSql := stSql + ' YR_KEY integer,';        //키
    stSql := stSql + ' EM_CODE text(50),'; //사번
    stSql := stSql + ' EM_NAME text(100),';     //이름
    stSql := stSql + ' YR_DOORNO text(10),';       //출입문아이디
    stSql := stSql + ' YR_STARTTIME text(17),';    //시작시간
    stSql := stSql + ' YR_ENDTIME text(17),';      //종료시간
    stSql := stSql + ' CA_CARDNO text(30),';    //카드번호
    stSql := stSql + ' IS_USED text(2),';    //사용유무
    stSql := stSql + ' CO_JIJUMNAME text(100),';      //단과대명
    stSql := stSql + ' CO_DEPARTNAME text(100),';  //학과명
    stSql := stSql + ' YR_InsertTime text(17),';    //외출조작버튼
    stSql := stSql + ' PRIMARY KEY (SEQ) ';
    stSql := stSql + ' ) ';
  end else if UpperCase(G_stDBType) = 'FB' then
  begin
    stSql := 'Create Table TB_YSReserveHis (';
    stSql := stSql + ' SEQ integer NOT NULL,'; //순서
    stSql := stSql + ' YR_KEY integer,';        //키
    stSql := stSql + ' EM_CODE varchar(50),'; //사번
    stSql := stSql + ' EM_NAME varchar(100),';     //이름
    stSql := stSql + ' YR_DOORNO varchar(10),';       //출입문아이디
...
begin
  ...
  stSql := stSql + ' where CO_CONFIGGROUP = ''RESERVE'' ';
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('RESERVE','TABLE_VER','1','예약연동테이블');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','PGTYPE','1','1:연세대');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','DBTYPE','mysql','');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','DBIP','127.0.0.1','접속IP');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','DBPORT','3306','접속PORT');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','DBUSERID','root','접속ID');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','DBUSERPW','','접속PW');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','DBNAME','','데이터베이스명');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('RESERVE','RELAYTIME','5','연동시간(초)');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('EMRELAY','DBTYPE','MSSQL','');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('EMRELAY','DBIP','127.0.0.1','접속IP');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('EMRELAY','DBPORT','1433','접속PORT');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('EMRELAY','DBUSERID','sa','접속ID');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('EMRELAY','DBUSERPW','sapasswd','접속PW');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := InsertIntoTB_CONFIG('EMRELAY','DBNAME','zmos','데이터베이스명');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := AlterTB_ACCESSEVENT_ACRelayAdd;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := AlterTB_DOOR_YRDOORNO;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := CreateTB_YSReserveHis;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := CreateTB_YSReserveDOOR;
  dmDBModule.ProcessExecSQL(stSql);
