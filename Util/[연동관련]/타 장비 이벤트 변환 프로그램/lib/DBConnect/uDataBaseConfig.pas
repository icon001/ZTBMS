...
begin
  ...
  stSql := stSql + ' where CO_CONFIGGROUP = ''DEVCONV'' ';
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('DEVCONV','TABLE_VER','1','장비변환테이블');
...
var
  stSql : string;
begin
  if G_stDBType = 'MDB' then
  begin
    stSql := 'Create Table TB_RELAYDEVICEMAPPINGCODE (';
    stSql := stSql + ' RM_CODE text(20) NOT NULL,';
    stSql := stSql + ' RM_ECUID text(2) NULL,';
    stSql := stSql + ' RM_DOORID text(2) NULL,';
    stSql := stSql + ' RM_READERID text(2) NULL,';
    stSql := stSql + ' PRIMARY KEY (RM_CODE) ';
    stSql := stSql + ' ) ';
  end else if G_stDBType = 'PG' then
  begin
    stSql := 'Create Table TB_RELAYDEVICEMAPPINGCODE (';
    stSql := stSql + ' RM_CODE varchar(20) NOT NULL,';
    stSql := stSql + ' RM_ECUID varchar(2) NULL,';
    stSql := stSql + ' RM_DOORID varchar(2) NULL,';
    stSql := stSql + ' RM_READERID varchar(2) NULL,';
    stSql := stSql + ' PRIMARY KEY (RM_CODE) ';
    stSql := stSql + ' ) ';
  end else if G_stDBType = 'MSSQL' then
  begin
    stSql := 'Create Table TB_RELAYDEVICEMAPPINGCODE (';
    stSql := stSql + ' RM_CODE varchar(20) NOT NULL,';
    stSql := stSql + ' RM_ECUID varchar(2) NULL,';
    ...
    stSql := stSql + ' PRIMARY KEY (RM_CODE) ';
    stSql := stSql + ' ) ';
  end else if G_stDBType = 'FB' then
  begin
    stSql := 'Create Table TB_RELAYDEVICEMAPPINGCODE (';
    stSql := stSql + ' RM_CODE varchar(20) NOT NULL,';
    stSql := stSql + ' RM_ECUID varchar(2) ,';
    stSql := stSql + ' RM_DOORID varchar(2) ,';
    stSql := stSql + ' RM_READERID varchar(2) ,';
    stSql := stSql + ' PRIMARY KEY (RM_CODE) ';
    stSql := stSql + ' ) ';
  end else Exit;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','2');
  dmDBModule.ProcessExecSQL(stSql);
...
begin
  if G_stDBType = 'MDB' then
  begin
    stSql := 'Create Table TB_RELAYEVENT (';
    stSql := stSql + ' RM_CODE text(20) NOT NULL,';
    stSql := stSql + ' RE_DATE text(14) NOT NULL,';
    stSql := stSql + ' RE_CARDNO text(30) NOT NULL,';
    stSql := stSql + ' RE_SEND text(1) NULL,';
    stSql := stSql + ' PRIMARY KEY (RM_CODE,RE_DATE,RE_CARDNO) ';
    stSql := stSql + ' ) ';
  end else if G_stDBType = 'PG' then
  begin
    stSql := 'Create Table TB_RELAYEVENT (';
    stSql := stSql + ' RM_CODE varchar(20) NOT NULL,';
    stSql := stSql + ' RE_DATE varchar(14) NOT NULL,';
    stSql := stSql + ' RE_CARDNO varchar(30) NOT NULL,';
    stSql := stSql + ' RE_SEND varchar(1) NULL,';
    stSql := stSql + ' PRIMARY KEY (RM_CODE,RE_DATE,RE_CARDNO) ';
    stSql := stSql + ' ) ';
  end else if G_stDBType = 'MSSQL' then
  begin
  ...
  begin
    ...
    stSql := stSql + ' RE_DATE varchar(14) NOT NULL,';
    stSql := stSql + ' RE_CARDNO varchar(30) NOT NULL,';
    stSql := stSql + ' RE_SEND varchar(1),';
    stSql := stSql + ' PRIMARY KEY (RM_CODE,RE_DATE,RE_CARDNO) ';
    stSql := stSql + ' ) ';
  end else Exit;
  dmDBModule.ProcessExecSQL(stSql);

  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','3');
  dmDBModule.ProcessExecSQL(stSql);
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('DEVCONV','INCARD','0','0.HEX,1.decimal,2.hid,3.ascii');
  dmDBModule.ProcessExecSQL(stSql);
  stSql := InsertIntoTB_CONFIG('DEVCONV','SAVECARD','0','0.HEX,1.ascii');
  dmDBModule.ProcessExecSQL(stSql);

  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','4');
  dmDBModule.ProcessExecSQL(stSql);
end;
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('DEVCONV','INDEVTYPE','0','0.등록기,1.SpeedGate');
  dmDBModule.ProcessExecSQL(stSql);
  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','5');
  dmDBModule.ProcessExecSQL(stSql);
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('DEVCONV','INCOMPORT','0','포트번호');
  dmDBModule.ProcessExecSQL(stSql);
  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','6');
  dmDBModule.ProcessExecSQL(stSql);
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('DEVCONV','SERVERPORT','7777','서버포트');
  dmDBModule.ProcessExecSQL(stSql);
  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','7');
  dmDBModule.ProcessExecSQL(stSql);
...
var
  stSql : string;
begin
  stSql := InsertIntoTB_CONFIG('DEVCONV','INCDLENTP','1','입력카드타입');
  dmDBModule.ProcessExecSQL(stSql);
  stSql := UpdateTB_CONFIG('DEVCONV','TABLE_VER','8');
  dmDBModule.ProcessExecSQL(stSql); 
end;
