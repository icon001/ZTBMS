...
begin
  ...
  begin
    aLeavePacket := aData;
    Exit; //ETX 가 없으면 데이터가 모두 들어오지 않은 상태
  end;
  if nIndex > 1 then
  begin
    Result := copy(aData,2,nIndex - 1);   //STX,ETX 없애자
...
begin
  ...
  if aPacket[1] = 'C' then
  begin
    stDateTime := copy(aPacket,2,14);
    if G_nInputCardLengthType = 0 then stCardNo := copy(aPacket,16,8)
    else if G_nInputCardLengthType = 1 then stCardNo := copy(aPacket,16,16)
    else if G_nInputCardLengthType = 2 then
    begin
      stLen := copy(aPacket,16,2);
      stCardNo := copy(aPacket,18,strtoint(stLen));
    end;
