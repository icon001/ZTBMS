...
type
  ...
  private
    ...
    Function RelayCardSystem : Boolean;
...
begin
  ...
  RelayTimer.Interval := 60 * 10 * 1000; //10분에 한번씩 연동 하자.
  RelayConnect;
  UnivercityConnect;
  Try
...
function TfmMain.RelayCardSystem: Boolean;
var
  stSql : string;
  nFdmsID : integer;
  stJijumName : string;
...
begin
  ...
  result := False;
  stSql := ' Select * from MFC_HUB ';
  stSql := stsql + ' where HUB_YN is null ';
  stSql := stsql + ' OR HUB_YN =''N'' ';
  stSql := stSql + ' order by HUB_SEQNO ';
  ...
  begin
    ...
    while Not Eof do
    begin
      stCardNo:= FindField('hub_chipserno').AsString;
      stEmCode:= FindField('HUB_IDNO').AsString;
      stCardType:= FindField('HUB_LSFG').AsString;  //1.분실
    ...
    begin
      DongYangDelete_MFC_HUB_SEQ(inttostr(strtoint(stEMSeq) - 3000));
    end;
  end;
end;
...
begin
  ...
  aCardType := '0';
  aCardSeq := '0';
  stSql := 'select  * from MFC_USM where usm_idno = ''' + aEmCode + ''' order by to_number(usm_isfg) desc';
  Try
    CoInitialize(nil);
    TempAdoQuery := TADOQuery.Create(nil);
