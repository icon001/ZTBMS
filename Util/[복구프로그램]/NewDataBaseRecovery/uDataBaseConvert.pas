...
begin
  ...
  if UpperCase(targetName) = 'TB_EMPLOYEECHANGE' then
  begin
    ga_Table.Progress := ga_Table.MaxValue;
    Ga_Tot.Progress := Ga_Tot.Progress + 1;
    result := True; //TB_EMPLOYEECHANGE 테이블은 복구 하지 말자. Serial 부분 때문에 문제 발생함
    Exit;
  end;
  if UpperCase(targetName) = 'TB_EMPLOYEEEXPIRECHANGE' then
  begin
    ga_Table.Progress := ga_Table.MaxValue;
    Ga_Tot.Progress := Ga_Tot.Progress + 1;
    result := True; //TB_EMPLOYEEEXPIRECHANGE 테이블은 복구 하지 말자. Serial 부분 때문에 문제 발생함
    Exit;
  end;
  ...
  if UpperCase(targetName) = 'TB_KTTMAPPINGCODE' then
  begin
    ga_Table.Progress := ga_Table.MaxValue;
    Ga_Tot.Progress := Ga_Tot.Progress + 1;
    result := True; //TB_KTTMAPPINGCODE 테이블은 복구 하지 말자.
    Exit;
  end;
...
{  if Not DropTable(targetName) then
  ...
  begin
    ...
    memo1.Lines.Add(targetName +' 테이블 복구 실패');
    Ga_Tot.Progress := Ga_Tot.Progress + 1;
    Exit;
  end;
...
begin
  ...
  Try
    ...
    if(  (Pos('SERIAL',UpperCase(stSql)) > 0 ) or (Pos('IDENTITY',UpperCase(stSql)) > 0 ) ) then result := false
    else  result := True;
  Finally
