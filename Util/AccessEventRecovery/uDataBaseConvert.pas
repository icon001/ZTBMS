...
type
  ...
  public
    ...
    Function copyTable(aSourceTableName,aTargetTableName:string):Boolean;
  end;
...
{  if Not DropTable(targetName) then
  ...
  begin
    DropTable(targetName  + 'bak');
    if Not CreateTable(targetName) then
    begin
      showmessage(targetName  + 'bak' +' 테이블 생성 오류입니다.');
      Exit;
    end;
    stSql := 'copy ' + targetName  + 'bak' + ' From ''' + stringReplace(aImportFileName,'\','\\',[rfReplaceAll]) + '''';
    with TblImport do
    begin
      Close;
      //Sql.Clear;
    ...
    copyTable(targetName  + 'bak',targetName);
  end;
  Screen.Cursor                    := crDefault;
  Ga_Tot.Progress := Ga_Tot.Progress + 1;
  result := True;
...
begin
  ...
  Ga_Tot.MaxValue := 1;
  Ga_Tot.Progress := 0;
  if Not FileDBRestor(ed_BackupDir.Text + '\TB_ACCESSEVENT.BAK' ) then
  begin
    Memo1.Lines.Add('TB_ACCESSEVENT.BAK 복구 실패');
    showmessage('TB_ACCESSEVENT.BAK 복구 실패');
  end;
  ...
  Try
    ...
    begin
      ...
      begin
        ...
        stSql := StringReplace(stSql,aTableName,aTableName + 'bak',[rfReplaceAll]);
...
function TForm1.copyTable(aSourceTableName,
  aTargetTableName: string): Boolean;
var
  stSql : string;
  i : integer;
  stFieldName : string;
begin
  result := False;
  stSql := 'select * from ' + aSourceTableName ;
  ...
  begin
    ...
    while Not Eof do
    begin
      for i := 0 to targetADOQuery.FieldCount - 1 do
      begin
        stFieldName := targetADOQuery.Fields[i].Name;
      end;
      next;
    end;
    result := True;
