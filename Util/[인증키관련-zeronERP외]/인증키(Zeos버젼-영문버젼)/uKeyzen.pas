...
uses
  ...
  DB, ADODB,DIMime, IdTCPClient, IdHTTP, Buttons,DateUtils;
...
//  stMac := UpperCase(GetMacAddress);
  ...
  begin
    ...
    if nMaxCount = 0 then
    begin
      showmessage('It is the subscriber is not registered.');//msk_SaupId.Text + '는 등록되지 않은 가입자 입니다. ');
      Exit;
    end;
    ...
    if Not bResult then
    begin
      showmessage('Password is invalid');//'비밀번호가 올바르지 않습니다.');
      Exit;
    end;
    ...
    begin
      ...
      begin
        showmessage('I have exceeded the counter.');//msk_SaupId.Text + '는 설치 대수를 초과하였습니다.(' + inttostr(nMaxCount) + ')');
        Exit;
      end;
      ...
      begin
        showmessage('Authentication Extraction error ');//'인증번호 추출 에러입니다.네트웍을 확인하여 주세요');
        Exit;
      end;
    end else
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        showmessage('Bad value for authentication key '); //'인증키값이 틀립니다. 정확한 인증키값을 넣어 주세요.');
        Exit;
      end;
    end;
  end;
...
{  if L_bConnected then
  ...
  LMDIniCtrl1.WriteString('Company','ID',msk_SaupId.Text);
  LMDIniCtrl1.WriteString('Company','Name',ed_Coname.Text);
  LMDIniCtrl1.WriteString('Company','Key'+L_stGubun,stKey);
  LMDIniCtrl1.WriteString('Company','Date'+L_stGubun,stDate);
  LMDIniCtrl1.WriteString('Company','MAC',stMac);
...
begin
  ...
  begin
    ...
    dtTime :=EncodeDateTime(strtoint(stYYYY),strtoint(stMM),strtoint(stDD),00,00,00,0);
