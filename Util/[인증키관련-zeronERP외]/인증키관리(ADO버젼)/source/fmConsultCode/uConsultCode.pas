...
begin
  ...
  begin
    ...
    begin
      ...
      while Not Eof do
      begin
        cells[0,nRow] := FindField('AC_CONSULTCODE').AsString;
        cells[1,nRow] := FindField('AC_CONSULTNAME').AsString;
        if FindField('AC_CONSULTCODE').AsString  = aCode then
        begin
          SelectRows(nRow,1);
        end;
...
begin
  ...
  result := False;
  stSql := 'Delete From TB_AUTHCONSULTCODE ';
  stSql := stSql + ' where AC_CONSULTCODE = ''' + aCode + '''';
...
begin
  ...
  stSql := ' Insert Into TB_AUTHCONSULTCODE( ';
  stSql := stSql + ' AC_CONSULTCODE,';
  stSql := stSql + ' AC_CONSULTNAME )' ;
  stSql := stSql + ' values( ';
  stSql := stSql + '''' + aCode + ''',';
  stSql := stSql + '''' + aName + '''';
  stSql := stSql + ')';
...
begin
  ...
  stSql := stSql + ' AC_CONSULTNAME = ''' + aName + ''' ';
  stSql := stSql + ' where AC_CONSULTCODE =''' + aCode + '''';
