...
//  ATTENDTYPE : String; //근태사용타입(BUTTON/UPDATE/READERNO(1/2))
  ...
  SendPersonRelayType : integer; //보내는 사원연동형태 0:연동안함,1:삼육대 연동,2:TDK근태
...
//  if SaveLogData < 1 then Exit;
  ...
  aFileName:= G_stExeFolder + '\..\log\DBerr'+ FormatDateTIme('yyyymmdd',Now)+'.log';
...
var
  stSql : string;
begin
  if aUserID = '' then aUserID := G_stMasterID ;
