...
{    Function CreateTB_ALARMGUBUNCODE : string;
    ...
    Function CreateTB_SERVERCARDRELAY:string;
    Function CreateTB_SERVERCARDRELAYHIS:string;
    Function CreateTB_SMSALARMSTATE:string;
    Function CreateTB_SMSALARMZONE:string;
    Function CreateTB_SMSUSER:string;
    Function CreateTB_WORKLOG:string;
    ...
    Function AlterTB_ALARMEVENTOperChange : string;
    Function AlterTB_ALARMEVENTSMSSEND_Add : string;
    Function AlterTB_ALARMSTATUSCODESMSSEND_Add : string;
    Function AlterTB_ALARMEVENTSTATECODE2Add : string;
    Function AlterTB_ALARMSTATUSCODEALARMSOUND : string;
    Function AlterTB_ALARMSTATUSCODECodeChange :string;
...
//    Function AlterTB_ALARMSTATUSCODEPrimaryDelete : string;
    ...
    Function AlterTB_READERReaderVer_Add : string;
    Function AlterTB_READERRegSend_Add : string;
    Function AlterTB_SMSALARMZONEName_Add : string;
    Function AlterTB_ZONEDEVICEUPDATE : string;
    Function AlterTB_ZONEDEVICEDelayUse_Add : string;
    Function AlterTB_ZONEDEVICEPortRecovery_Add : string;
    Function AlterTB_ZONEDEVICERegSend_Add : string;
...
begin
  ...
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,bb.AD_USERID ';
  end;
  stSql := stSql + ' from TB_Door ba ' ;
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then
    begin
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,bd.AD_USERID ';
  end;
...
{  stSql := stSql + ' Inner Join TB_ACCESSDEVICE e ';
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,bg.AD_USERID ';
  end;
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 1 then
    begin
      stSql := stSql + ' AND a.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
    end else if G_nBuildingGrade = 2 then
    begin
      stSql := stSql + ' AND a.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
      stSql := stSql + ' AND a.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
    end else if G_nBuildingGrade = 3 then
    begin
      stSql := stSql + ' AND a.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
      stSql := stSql + ' AND a.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
      stSql := stSql + ' AND a.LO_AREACODE = ''' + G_stMasterAreaCode + ''' ';
    end else if G_nBuildingGrade = 4 then stSql := stSql + ' AND b.AD_USERID = ''' + G_stMasterID + ''' ';
  end;
...
begin
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ( ';
  end;

  stSql := stSql + ' (select * from TB_ALARMEVENT IN ''';
  stSql := stSql + G_stExeFolder + '\..\DB\ZEVENT.mdb'') a ';
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then
    begin
      stSql := stSql + ' INNER JOIN (select * from TB_ADMINALARMDEVICE ';
      stSql := stSql + ' Where AD_USERID = ''' + G_stMasterID + ''') b ';
...
{  stSql := stSql + ' Inner Join TB_ACCESSDEVICE be ';
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,bf.AD_USERID ';
  end;
...
begin
  ...
  stSql := stSql + ' (select * from TB_ALARMEVENT IN ''';
  stSql := stSql + G_stExeFolder + '\..\DB\ZEVENT.mdb'' ';
  if Trim(aAlarmType) <> '' then
  begin
...
begin
  ...
  stSql := stSql + '''' + G_stExeFolder + '\..\DB\ZMOS.mdb'' ';
...
begin
  ...
  begin
    ...
    begin
      stSql := stSql + ' Inner Join (select * from TB_ADMINALARMDEVICE ';
      stSql := stSql + ' Where AD_USERID = ''' + G_stMasterID + ''') c ';
      stSql := stSql + ' ON (a.GROUP_CODE = c.GROUP_CODE ';
      stSql := stSql + ' AND a.AC_NODENO = c.AC_NODENO ';
      stSql := stSql + ' AND a.AC_ECUID = c.AC_ECUID ) ';
      stSql := stSql + ' ) ';
...
begin
  ...
  begin
    ...
    begin
      stSql := stSql + ' Inner Join (select * from TB_ADMINALARMDEVICE ';
      stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
      stSql := stSql + ' AND AD_USERID = ''' + G_stMasterID + ''') c ';
      stSql := stSql + ' ON (a.GROUP_CODE = c.GROUP_CODE ';
      stSql := stSql + ' AND a.AC_NODENO = c.AC_NODENO ';
      stSql := stSql + ' AND a.AC_ECUID = c.AC_ECUID ) ';
      stSql := stSql + ' )';
...
begin
  ...
  begin
    ...
    begin
      stSql := stSql + ' Inner Join (select * from TB_ADMINALARMDEVICE ';
      stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
      stSql := stSql + ' AND AD_USERID = ''' + G_stMasterID + ''') d ';
      stSql := stSql + ' ON (a.GROUP_CODE = d.GROUP_CODE ';
      stSql := stSql + ' AND a.AC_NODENO = d.AC_NODENO ';
      stSql := stSql + ' AND a.AC_ECUID = d.AC_ECUID ) ';
      stSql := stSql + ' )  ';
...
//  stSql := stSql + ' ( ';
  ...
  stSql := stSql + G_stExeFolder + '\..\DB\ZEVENT.mdb'' where AT_DATE =''' + aDate + ''') f  ';
...
{  stSql := stSql + ' Left JOIN TB_CARD b ';
  ...
  stSql := stSql + G_stExeFolder + '\..\DB\ZEVENT.mdb'' where AT_DATE Between ''' + aFromDate + ''' ';
...
begin
  ...
  begin
    ...
    begin
      stSql := stSql + ' Inner Join (select * from TB_ADMINDOOR ';
      stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
      stSql := stSql + ' AND AD_USERID = ''' + G_stMasterID + ''' ';
      stSql := stSql + ' ) b ';
      stSql := stSql + ' ON (a.GROUP_CODE = b.GROUP_CODE ';
      stSql := stSql + ' AND a.AC_NODENO = b.AC_NODENO ';
      stSql := stSql + ' AND a.AC_ECUID = b.AC_ECUID ';
...
begin
  ...
  begin
    ...
    begin
      stSql := stSql + ' Inner Join (select * from TB_ADMINDOOR ';
      stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
      stSql := stSql + ' AND AD_USERID = ''' + G_stMasterID + ''') b ';
      stSql := stSql + ' ON (a.GROUP_CODE = b.GROUP_CODE ';
      stSql := stSql + ' AND a.AC_NODENO = b.AC_NODENO ';
      stSql := stSql + ' AND a.AC_ECUID = b.AC_ECUID ';
      stSql := stSql + ' AND a.DO_DOORNO = b.DO_DOORNO )';
...
begin
  ...
  stSql := stSql + ' ) ';
  stSql := stSql + ' Left JOIN  (select * from TB_ATEVENT IN ''';
  stSql := stSql + G_stExeFolder + '\..\DB\ZEVENT.mdb'') f ';
...
begin
  ...
  stSql := stSql + ' Left JOIN (select * from TB_ATEVENT IN ''';
  stSql := stSql + G_stExeFolder + '\..\DB\ZEVENT.mdb'' where AT_DATE =''' + aDate + ''') a ';
...
begin
  ...
  stSql := stSql + '     a.DE_UPDATEOPERATOR = ''' + G_stMasterID + ''' ';
...
function TMDBSql.AlterTB_ALARMEVENTSMSSEND_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_ALARMEVENT ADD AL_SMSSEND text(1) NULL ';
  result := stSql;
end;

function TMDBSql.CreateTB_SMSALARMZONE: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SMSALARMZONE (';
  stSql := stSql + ' GROUP_CODE text(10) DEFAULT ''1234567890'' NOT NULL,';
  stSql := stSql + ' AC_NODENO int NOT NULL,';
  stSql := stSql + ' AC_ECUID text(2) NOT NULL,';
  stSql := stSql + ' AL_ZONENUM text(1) NOT NULL,';
  stSql := stSql + ' SA_TELNO text(20) NOT NULL,';
  stSql := stSql + ' SA_USE text(1)  NULL,';
  stSql := stSql + ' SA_MESSAGE text(200)  NULL,';
  stSql := stSql + ' PRIMARY KEY (GROUP_CODE, AC_NODENO,';
  stSql := stSql + ' AC_ECUID,AL_ZONENUM,SA_TELNO) ';
  stSql := stSql + ' ) ';

  result := stSql;
end;

function TMDBSql.AlterTB_ALARMSTATUSCODESMSSEND_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_ALARMSTATUSCODE ADD AL_SMSSEND text(1) NULL ';
  result := stSql;

end;

function TMDBSql.AlterTB_SMSALARMZONEName_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_SMSALARMZONE ADD SA_NAME text(100) NULL ';
  result := stSql;

end;

function TMDBSql.CreateTB_SMSALARMSTATE: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SMSALARMSTATE (';
  stSql := stSql + ' GROUP_CODE text(10) DEFAULT ''1234567890'' NOT NULL,';
  stSql := stSql + ' AL_ALARMSTATUSCODE text(5) NOT NULL,';
  stSql := stSql + ' SA_TELNO text(20) NOT NULL,';
  stSql := stSql + ' SA_NAME text(100) NULL,';
  stSql := stSql + ' SA_USE text(1)  NULL,';
  stSql := stSql + ' SA_MESSAGE text(200)  NULL,';
  stSql := stSql + ' PRIMARY KEY (GROUP_CODE, AL_ALARMSTATUSCODE,SA_TELNO) ';
  stSql := stSql + ' ) ';
...
function TMDBSql.CreateTB_SMSUSER: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SMSUSER (';
  stSql := stSql + ' GROUP_CODE text(10) DEFAULT ''1234567890'' NOT NULL,';
  stSql := stSql + ' SA_TELNO text(30) NOT NULL,';
  stSql := stSql + ' SA_NAME text(50) NULL,';
  stSql := stSql + ' PRIMARY KEY (GROUP_CODE, SA_TELNO) ';
  stSql := stSql + ' ) ';
