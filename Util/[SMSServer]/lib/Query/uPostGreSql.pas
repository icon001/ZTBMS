...
begin
  ...
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,a.AD_USERID ';
  end;
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,c.AD_USERID ';
  end;
...
{  stSql := stSql + ' Inner Join TB_ACCESSDEVICE e ';
  ...
  if Not G_bIsMaster then
  begin
    if G_nBuildingGrade = 4 then stSql := stSql + ' ,f.AD_USERID ';
  end;
...
begin
  ...
  begin
    if G_nBuildingGrade = 1 then
    begin
      stSql := stSql + ' AND c.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
    end else if G_nBuildingGrade = 2 then
    begin
      stSql := stSql + ' AND c.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
      stSql := stSql + ' AND c.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
    end else if G_nBuildingGrade = 3 then
    begin
      stSql := stSql + ' AND c.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
      stSql := stSql + ' AND c.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
      stSql := stSql + ' AND c.LO_AREACODE = ''' + G_stMasterAreaCode + ''' ';
    end else if G_nBuildingGrade = 4 then stSql := stSql + ' AND b.AD_USERID = ''' + G_stMasterID + ''' ';
  end;
...
begin
  ...
  begin
    if G_nBuildingGrade = 1 then
    begin
      stSql := stSql + ' AND b.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
    end else if G_nBuildingGrade = 2 then
    begin
      stSql := stSql + ' AND b.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
      stSql := stSql + ' AND b.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
    end else if G_nBuildingGrade = 3 then
    begin
      stSql := stSql + ' AND b.LO_DONGCODE = ''' + G_stMasterBuildingCode + ''' ';
      stSql := stSql + ' AND b.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
      stSql := stSql + ' AND b.LO_AREACODE = ''' + G_stMasterAreaCode + ''' ';
    end else if G_nBuildingGrade = 4 then stSql := stSql + ' AND b.AD_USERID = ''' + G_stMasterID + ''' ';
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if G_nBuildingGrade = 4 then  stSql := stSql + ' AND c.AD_USERID = ''' + G_stMasterID + ''' ';
        if (aBuildingCode <> '') and (aBuildingCode <> '000') then
          stSql := stSql + ' AND a.LO_DONGCODE = ''' + aBuildingCode + ''' ';
        if (aFloorCode <> '') and (aFloorCode <> '000') then
          stSql := stSql + ' AND a.LO_FLOORCODE = ''' + aFloorCode + ''' ';
...
{  stSql := stSql + ' Inner Join TB_ACCESSDEVICE b ';
  ...
  begin
    ...
    begin
      ...
      stSql := stSql + ' AND a.LO_FLOORCODE = ''' + G_stMasterFloorCode + ''' ';
      stSql := stSql + ' AND a.LO_AREACODE = ''' + G_stMasterAreaCode + ''' ';
    end else if G_nBuildingGrade = 4 then stSql := stSql + ' AND c.AD_USERID = ''' + G_stMasterID + ''' ';
  end;
...
{  stSql := stSql + ' INNER JOIN TB_ACCESSDEVICE b ';
  ...
  begin
    ...
    begin
      if G_nBuildingGrade = 4 then stSql := stSql + ' AND d.AD_USERID = ''' + G_stMasterID + ''' ';
      if (aBuildingCode <> '') and (aBuildingCode <> '000') then
        stSql := stSql + ' AND a.LO_DONGCODE = ''' + aBuildingCode + ''' ';
      if (aFloorCode <> '') and (aFloorCode <> '000') then
        stSql := stSql + ' AND a.LO_FLOORCODE = ''' + aFloorCode + ''' ';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if G_nBuildingGrade = 4 then stSql := stSql + ' AND b.AD_USERID = ''' + G_stMasterID + ''' ';
        if (aBuildingCode <> '') and (aBuildingCode <> '000') then
          stSql := stSql + ' AND a.LO_DONGCODE = ''' + aBuildingCode + ''' ';
        if (aFloorCode <> '') and (aFloorCode <> '000') then
          stSql := stSql + ' AND a.LO_FLOORCODE = ''' + aFloorCode + ''' ';
...
begin
    ...
    stSql := stSql + ' DE_UPDATEOPERATOR = ''' + G_stMasterID + ''' ';
...
function TPostGreSql.AlterTB_ALARMEVENTSMSSEND_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_ALARMEVENT ADD AL_SMSSEND char(1) DEFAULT ''N'' NULL ';
  result := stSql;

end;

function TPostGreSql.CreateTB_SMSALARMZONE: string;
var
  stSql : string;
begin
  ...
  stSql := stSql + ' SA_TELNO varchar(20) NOT NULL,';
  stSql := stSql + ' SA_USE char(1)  NULL,';
  stSql := stSql + ' SA_MESSAGE varchar(200)  NULL,';
  stSql := stSql + ' PRIMARY KEY (GROUP_CODE, AC_NODENO,';
...
function TPostGreSql.AlterTB_ALARMSTATUSCODESMSSEND_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_ALARMSTATUSCODE ADD AL_SMSSEND char(1) NULL ';
  result := stSql;
end;

function TPostGreSql.AlterTB_SMSALARMZONEName_Add: string;
var
  stSql : string;
begin
  stSql := 'ALTER TABLE TB_SMSALARMZONE ADD SA_NAME varchar(100) NULL ';
  result := stSql;

end;

function TPostGreSql.CreateTB_SMSALARMSTATE: string;
var
  stSql : string;
begin
  ...
  stSql := stSql + ' AL_ALARMSTATUSCODE varchar(5) NOT NULL,';
  stSql := stSql + ' SA_TELNO varchar(20) NOT NULL,';
  stSql := stSql + ' SA_NAME varchar(100) NULL,';
  stSql := stSql + ' SA_USE varchar(1)  NULL,';
  stSql := stSql + ' SA_MESSAGE varchar(200)  NULL,';
...
function TPostGreSql.CreateTB_SMSUSER: string;
var
  stSql : string;
begin
  stSql := 'Create Table TB_SMSUSER (';
  stSql := stSql + ' GROUP_CODE varchar(10) DEFAULT ''1234567890'' NOT NULL,';
  stSql := stSql + ' SA_TELNO varchar(30) NOT NULL,';
  stSql := stSql + ' SA_NAME varchar(50) NULL,';
