...
type
  ...
  private
    ...
    Function CheckTB_EMPLOYEE_JijumCode(aEmCode:string;var aOldJijumCode:string):Boolean;
    ...
    Function CheckTB_CARD_EmCode(aEmCode:string):Boolean;
    ...
    Function UpdateTB_EMPLOYEE(aEmpID,aEmpNM,aCompanyCode,aJijumCode,aDepartCode,aPosiCode,aCompanyPhone,
                                 aJoinDate,aRetireDate,aZipcode,aAddr1,aAddr2,aHomePhone,aHandphone,
                                 aRegGubun,aCardNo,aEmpImg,afdmsId,aEmTypeCode:string):Boolean;
    Function UpdateTB_EMPLOYEE_Code(aEmCode,aJijumCode,aDepartCode,aPosiCode:string):Boolean;
  ...
  private
    ...
    Function GetTB_EMPLOYEE_Emcode(aEmName:string;var aEmCode:string):Boolean;
  public
    { Public declarations }
    ...
    procedure CardMapping;
    function  LineCardMapping(aLineData:string):Boolean;
  end;
...
begin
  ...
  if Not chk_CardMapping.Checked then
  begin
...
begin
  ...
  Try
    ...
    stJijumName := TempList.Strings[0];
    stDepartName := TempList.Strings[1];
    if TempList.Count > 7 then stPosiName := TempList.Strings[7]
    else stPosiName := '';
    ...
    if TempList.Count > 2 then stEmCode := TempList.Strings[2]
    else Exit;
    stCardNo := '';
    if TempList.Count > 8 then stCardNo := TempList.Strings[8]
    else stCardNo := '';
    if TempList.Count > 3 then stEmName := TempList.Strings[3]
    else stEmName := '';
    if TempList.Count > 4 then stHandPhone := TempList.Strings[4]
    else stHandPhone := '';
    if TempList.Count > 5 then stCompanyPhone := TempList.Strings[5]
    else stCompanyPhone := '';
    Label2.Caption :=  stEmCode;
    if Trim(stEmCode) = '' then Exit;

    if stCardNo <> '' then InsertTB_CARD(stCardNo,'1','1',stEmCode,'001');

    if CheckTB_EMPLOYEE_JijumCode(stEmCode,stOldJijumCode) then
    begin
      //EMPLOYEE 테이블에 사번이 존재 하면
      if (stOldJijumCode = '') or
         (stOldJijumCode = '000') then
      begin
        UpdateTB_EMPLOYEE_Code(stEmCode,stJijumCode,stDepartCode,stPosiCode);
        stSql :=  InsertIntoTB_EMPHIS('001',stEmCode,inttostr(aFdmsID),
                          '1',stCardNo,stCardState,stEmName,
                          '','연세대학교',stJijumName,stDepartName,stPosiName);
        ProcessExecSQL(stSql);
        result := True;
        Exit;
      end;
...
begin
  ...
  Try
    ...
    begin
      Gauge1.Progress := i;
      LineCardMapping(CardList.Strings[i]);
      Application.ProcessMessages;
      Label1.Caption := inttostr(i);
    end;
  Finally
...
function TfmMain.CheckTB_EMPLOYEE_JijumCode(aEmCode: string;
  var aOldJijumCode: string): Boolean;
var
  stSql: string;
  TempAdoQuery : TADOQuery;
begin
  ...
  Try
    ...
    begin
      ...
      if RecordCount < 1 then Exit;
      Result := True;
      aOldJijumCode := FindField('CO_JIJUMCODE').AsString;
    end;
  Finally
    TempAdoQuery.Free;
    CoUninitialize;
...
function TfmMain.UpdateTB_EMPLOYEE_Code(aEmCode, aJijumCode, aDepartCode,
...
function TfmMain.LineCardMapping(aLineData: string): Boolean;
var
  TempList : TStringList;
  i : integer;
  stCompanyCode : string;
...
begin
  ...
  Try
    aLineData := stringReplace(aLineData,' ','',[rfReplaceAll]);
    TempList := TStringList.Create;
    TempList.Delimiter := ',';
    TempList.DelimitedText := aLineData;
    ...
    if TempList.Count > 3 then stEmCode := TempList.Strings[3]
    else stEmCode := '';
    if Trim(stEmCode) = '' then
    begin
      GetTB_EMPLOYEE_Emcode(stEmName,stEmCode);
      if stEmCode = '' then Exit;
    end;

    if Not CheckTB_EMPLOYEE('001',stEmCode) then Exit;

    if Not CheckTB_CARD(stCardNo) then
    begin
      result := InsertTB_CARD(stCardNo,'1','1',stEmCode,'001');
    end;
...
function TfmMain.GetTB_EMPLOYEE_Emcode(aEmName: string;
...
begin
  ...
  aEmCode := '';
  stSql := 'select * from TB_EMPLOYEE ';
  stSql := stSql + ' Where EM_NAME = ''' + aEmName + ''' ';
  ...
  begin
    ...
    while Not Eof do
    begin
      if Not CheckTB_CARD_EmCode(FindField('EM_CODE').AsString) then
      begin
        aEmCode := FindField('EM_CODE').AsString;
        result := True;
        Exit;
...
function TfmMain.CheckTB_CARD_EmCode(aEmCode: string): Boolean;
var
  stSql: string;
  TempAdoQuery : TADOQuery;
begin
