...
//식당데이터를 일별 상세조회 하는 쿼리
function SelectFoodDataDayDetail:string;
//식당데이터 일별 식수구분에 의한 집계내역 조회 하는 쿼리
...
begin
  ...
  stSql := stSql + ' CASE ';
  stSql := stSql + ' WHEN B.FOOD_CODE =''1'' THEN ''조식'' ';
  stSql := stSql + ' WHEN B.FOOD_CODE =''2'' THEN ''중식'' ';
  stSql := stSql + ' WHEN B.FOOD_CODE =''3'' THEN ''석식'' ';
  stSql := stSql + ' WHEN B.FOOD_CODE =''4'' THEN ''야식'' ';
...
begin
  ...
  stSql := stSql + ' CASE ';
  stSql := stSql + ' WHEN a.FOOD_CODE = ''1'' THEN ''조식'' ';
  stSql := stSql + ' WHEN a.FOOD_CODE = ''2'' THEN ''중식'' ';
  stSql := stSql + ' WHEN a.FOOD_CODE = ''3'' THEN ''석식'' ';
  stSql := stSql + ' WHEN a.FOOD_CODE = ''4'' THEN ''야식'' ';
