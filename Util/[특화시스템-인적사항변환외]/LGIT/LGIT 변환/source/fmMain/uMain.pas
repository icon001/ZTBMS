...
begin
  UpdateTB_FORMNAME('001','소속코드');
  UpdateTB_FORMNAME('002','소속명칭');
  UpdateTB_FORMNAME('011','회사코드');
  UpdateTB_FORMNAME('012','회사명칭');
  UpdateTB_FORMNAME('021','부서코드');
  UpdateTB_FORMNAME('022','부서명칭');
...
begin
  Try
    ...
    stCompanyName := TempList.Strings[3];
    ...
    stCompanyCode := '002';
    stJijumCode := GetJijumCode(stCompanyCode,stCompanyName);
    if stJijumCode = '000' then
    begin
      if Trim(stCompanyName) <> '' then
        CreateJijumCode(stCompanyCode,stCompanyName,stJijumCode);
    end;
    stDepartCode := '000';
    stPosiCode := '000';
    stCardState := '1';
    stEmCode := TempList.Strings[1];
    stEmCode := stringReplace(stEmCode,'^',' ',[rfReplaceAll]);
    stCardNo := TempList.Strings[2];
    stCardNo := stringReplace(stCardNo,'^','',[rfReplaceAll]);
    stCardNo := stringReplace(stCardNo,'-','',[rfReplaceAll]);
    stCardNo := FillZeroStrnum(stCardNo,15);
    stTemp1 := copy(stCardNo,6,5);
    stTemp1 := Dec2Hex(strtoint(stTemp1),4);
    stTemp2 := copy(stCardNo,11,5);
    stTemp2 := Dec2Hex(strtoint(stTemp2),4);
    stCardNo := stTemp1 + stTemp2;

    stEmName := TempList.Strings[0];
    ...
    begin
        ...
        stSql := InsertIntoTB_EMPHIS(stCompanyCode,stEmCode,inttostr(aFdmsID),
                          ...
                          '','',stCompanyName,stDepartName,stPosiName);
        ProcessExecSQL(stSql);
    end else
    begin
        stSql :=  InsertIntoTB_EMPHIS(stCompanyCode,stEmCode,inttostr(aFdmsID),
...
begin
  ...
  InsertIntoTB_COMPANY(aCompanycode,aJijumCode,'','2',aJijumName);
end;
