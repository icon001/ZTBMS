...
type
  ...
  public
    ...
    procedure DeleteTB_EmpHis;
    procedure DeleteTB_DEVICECARDNO;
    ...
    function ChangHexCardNo(aCardNo:string):string;
  end;
...
begin
  ...
  stCardNo := TempList.Strings[0] + TempList.Strings[1];
  stCardNo := FillZeroStrNum(stCardNo,16);
  stCardNo1 := copy(stCardNo,1,11);
  stCardNo2 := copy(stCardNo,12,5);
  stCardNo := Dec2Hex64(strtoint64(stCardNo1),4) + Dec2Hex64(strtoint64(stCardNo2),4);
  //stCardNo := ChangHexCardNo(stCardNo); 
  stCardNo := FillCharString(stCardNo,'*',16);
  if CheckTB_CARD(stCardNo) then Exit;
  stEmName := TempList.Strings[2];

  Try
    stCompanyName := TempList.Strings[4];
    stCompanyName := stringReplace(stCompanyName,'^',' ',[rfReplaceAll]);
    stJijumName := '본사';
    stJijumName := stringReplace(stJijumName,'^',' ',[rfReplaceAll]);
    stDepartName := TempList.Strings[3];
...
function TfmMain.ChangHexCardNo(aCardNo:string): string;
var
  stTemp : string;
begin
  aCardNo := FillCharString(aCardNo,'0',8,True);
  stTemp := copy(aCardNo,7,2) + copy(aCardNo,5,2) + copy(aCardNo,3,2) + copy(aCardNo,1,2);
  result := stTemp;
end;

procedure TfmMain.DeleteTB_DEVICECARDNO;
var
  stSql : string;
begin
...
procedure TfmMain.DeleteTB_EmpHis;
var
  stSql : string;
begin
