...
begin
  ...
  begin
    ...
    memo1.Lines.LoadFromFile(ed_DBPath.Text);
  end;
end;
...
begin
  ...
  Try
    ...
    if(stEmCode='') then
    begin
      stEmCode := GetNextEmCode();
    end;
    if(stEmCode='') then Exit;
    if TempList.Count > 4 then stEmName := TempList.Strings[4]
    else stEmName := '';
    stCardNo := '';
    if TempList.Count > 5 then stCardNo := TempList.Strings[5]
    else stCardNo := '';

    if(stCardNo <> '') then
    begin
      stCardNo := copy(stCardNo,3,8);
...
function TfmMain.GetNextEmCode: string;
var
  stSql: string;
  TempAdoQuery : TADOQuery;
  stSabun : string;
