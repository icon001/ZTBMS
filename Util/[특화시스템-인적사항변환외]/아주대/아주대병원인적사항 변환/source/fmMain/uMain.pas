...
begin
  ...
  begin
    ...
    conStr := constr + 'User Id='+ ed_Userid.Text + ';';
    conStr := constr + 'password=' + ed_Passwd.Text;
  end else if rg_dbtype.ItemIndex = 2 then  //MDB
  begin
    conStr := 'Provider=Microsoft.Jet.OLEDB.4.0;';
    conStr := conStr + 'Data Source=' + ExtractFileDir(Application.ExeName) + '\..\db\ZMOS.mdb' + ';';
    conStr := conStr + 'Persist Security Info=True;';
    conStr := conStr + 'Jet OLEDB:Database ';
...
var
  stSql : string;
begin
  InsertIntoTB_COMPANY('001','000','000','1','아주대병원');
end;
...
begin
  Try
    ...
    if length(stEmCode) < 6 then
      stEmCode := FillZeroStrNum(stEmCode,6,True);
    stCardNo := FillZeroStrNum(stEmCode,13,False) + FillZeroStrNum('0',3,False);
    ...
    if Not InsertTB_EMPLOYEE(stEmCode,
      ...
      begin
        ...
        stSql := InsertIntoTB_EMPHIS('001',stEmCode,inttostr(aFdmsID),
                          ...
                          '','아주대병원',stJijumName,stDepartName,stPosiName);
        ProcessExecSQL(stSql);
      end else
      begin
        stSql :=  InsertIntoTB_EMPHIS('001',stEmCode,inttostr(aFdmsID),
...
begin
  if rg_dbtype.ItemIndex = 2 then Panel1.Visible := False
