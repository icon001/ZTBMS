...
begin
  ...
  self.FindSubForm('Main').FindCommand('FORMSHOW').Params.Values['VALUE'] := 'AccessReport';
...
begin
  ...
  LoadPosiCode(CompanyCodeList.Strings[cmb_Company.ItemIndex],PosiCodeList,TComboBox(cmb_Posi));
...
begin
  ...
  GetAccessPermit(TComboBox(cmb_AccessPermit));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LoadJijumCode(CompanyCodeList.Strings[nIndex],JijumCodeList,TComboBox(cmb_Jijum));
      end;
      cmb_Company.Enabled := False;
      LoadPosiCode(MasterCompany,PosiCodeList,TComboBox(cmb_Posi));
    end;
...
begin
  ...
  begin
    ...
    if cmb_Type.ItemIndex = 2 then
      LoadEmpType(EmpTypeCodeList,TComboBox(cmb_EmType))
    else
    begin
      cmb_EmType.Clear;
...
procedure TfmAccessReport.cmb_PositionEnter(Sender: TObject);
begin
  inherited;
  cmb_Position.Width := 197;
end;

procedure TfmAccessReport.cmb_PositionExit(Sender: TObject);
begin
  inherited;
  cmb_Position.Width := 97;
end;

procedure TfmAccessReport.CommandArrayCommandsTCloseExecute(
  Command: TCommand; Params: TStringList);
begin
  inherited;
  Close;
