...
type
  TfmDeviceCode = class(TfmASubForm)
    ...
    btn_TimeCode1: TBitBtn;
    btn_TimeCode0: TBitBtn;
  ...
  ARow: Integer; Rect: TRect; State: TGridDrawState);
    ...
    procedure btn_TimeCode0Click(Sender: TObject);
    procedure btn_TimeCode1Click(Sender: TObject);
  private
...
begin
  ...
  btn_TimeCode0.Visible := G_bTimeCodeUse;
  btn_TimeCode1.Visible := G_bTimeCodeUse;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          cells[21,nRow] := FindField('AC_JAVARATYPE').AsString;
          cells[22,nRow] := FindField('AC_CARDBYTE').AsString;
          cells[23,nRow] := FindField('AC_ARMAREAUSE').AsString;
          cells[24,nRow] := FindField('AC_TIMETYPE').AsString;
...
begin
  ...
  begin
    ...
    if cells[21,Row] <> '0' then chk_MCUJavara.Checked := True
    else chk_MCUJavara.Checked := False;
    if isDigit(cells[22,Row]) then cmb_CardType.ItemIndex := strtoint(cells[22,Row])
    else cmb_CardType.ItemIndex :=  CARDLENGTHTYPE;
    if cells[23,Row] = 'Y' then
    begin
...
begin
  ...
  begin
    ...
    if cells[21,Row] <> '0' then chk_ECUJavara.Checked := True
...
begin
    ...
    begin
      bResult := dmDBFunction.DupCheckTB_ACCESSDEVICE_MCUIP(ed_McuIp.Text,ed_McuPort.Text);
      if bResult then
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if dmDBFunction.InsertTB_ACCESSDEVICE(ed_NodeNo.Text,ed_mcuid.Text,FillZeroNumber(i,2),
                                   '',ed_McuPort.Text,stEcuName,stBuildingCode,
                                   stFloorCode,stAreaCode,'N',stTotWidth,stTotHeight,
                                   '0','0','1','0','1','0','0','N',inttostr(cmb_CardReaderType.ItemIndex),'1','N',inttostr(se_McuSeq.Value),inttostr(CARDLENGTHTYPE),'N','0') then
              begin
    ...
    begin
      ...
      begin
        bResult := dmDBFunction.UpdateTB_ACCESSDEVICE(ed_NodeNo.Text,
                                       ed_mcuid.Text,
                                       '00',
                                       ed_McuIp.Text,
                                       ed_McuPort.Text,
      ...
      begin
        bResult := dmDBFunction.UpdateTB_ACCESSDEVICE_Info(ed_NodeNo.Text,
                                       ed_mcuid.Text,
                                       '00',
                                       ed_McuIp.Text,
                                       ed_McuPort.Text,
    ...
    if chk_DoorName.Checked then
      UpdateDoorName(ed_NodeNo.Text,'00',ed_MCUname.Text,chk_DoorName.Checked); //메인컨트롤러의 출입문 명칭을 변경해 주자
...
begin
    ...
    begin
      ...
      begin
        bResult := dmDBFunction.UpdateTB_ACCESSDEVICE(inttostr(strtoint(stNodeNo)),
                                       '',
                                       FillZeroNumber(ed_Ecuno.Value,2),
                                       '',
                                       '',
      ...
      begin
        bResult := dmDBFunction.UpdateTB_ACCESSDEVICE_Info(inttostr(strtoint(stNodeNo)),
                                       '',
                                       FillZeroNumber(ed_Ecuno.Value,2),
                                       '',
                                       '',
...
begin
  ...
  begin
    ...
    btn_TimeCode0.Enabled := True;
    cmb_CardType.Enabled := True;
    if G_nArmAreaUse = 1 then cmb_McuArmAreaUse.Enabled := True;
...
procedure TfmDeviceCode.chk_MCUJavaraClick(Sender: TObject);
begin
  inherited;
...
procedure TfmDeviceCode.chk_ECUJavaraClick(Sender: TObject);
begin
  inherited;
...
procedure TfmDeviceCode.btn_TimeCode0Click(Sender: TObject);
begin
  inherited;
  fmDeviceTimeCode:= TfmDeviceTimeCode.Create(Self);
  fmDeviceTimeCode.NodeNo := ed_NodeNo.Text;
  fmDeviceTimeCode.ECUID := '00';
  fmDeviceTimeCode.SHowmodal;
  fmDeviceTimeCode.Free;
end;

procedure TfmDeviceCode.btn_TimeCode1Click(Sender: TObject);
begin
  inherited;
  fmDeviceTimeCode:= TfmDeviceTimeCode.Create(Self);
  fmDeviceTimeCode.NodeNo := ed_NodeNo2.Text;
  fmDeviceTimeCode.ECUID := FillZeroNumber(ed_Ecuno.Value,2);
  fmDeviceTimeCode.SHowmodal;
  fmDeviceTimeCode.Free;
end;
