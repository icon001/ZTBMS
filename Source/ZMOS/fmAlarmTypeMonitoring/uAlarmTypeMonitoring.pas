...
begin
  ...
  //sg_AccessHeaderNameSet;  //2011.07.06 로직 변경에 의해 제외됨

  sg_Main.ColWidths[2] := 0;
  //if L_bFirst then       //20161226 FormActive 에서 수행 하던 것을 Showmessage 로 처리 함... //FormActive 에 있을때 화면에 로딩 되기 전에 데이터 들어 오는 부분 때문에 Index 에러 발생
  begin
    //L_bFirst := False;
    LoadingNode;
    LoadingEcu('000','전체');
...
//TEST___________
  ...
  //Form_Initialize;  //Ini 정보로 설정 하도록 처리함
...
begin
  ...
  begin
    ...
    self.FindSubForm('Main').FindCommand('SendData').Execute;
    DataModule1.TB_SYSTEMLOGInsert(copy(stAlarmAreaID,1,3),copy(stAlarmAreaID,4,2),ALARMTYPE,copy(stAlarmAreaID,6,1),'0000000000','경계모드')
  end;
end;
...
begin
  ...
  begin
    ...
    self.FindSubForm('Main').FindCommand('SendData').Execute;
    DataModule1.TB_SYSTEMLOGInsert(copy(stAlarmAreaID,1,3),copy(stAlarmAreaID,4,2),ALARMTYPE,copy(stAlarmAreaID,6,1),'0000000000','해제모드')
  end;
end;

procedure TfmAlarmTypeMonitoring.mn_DeviceChangeClick(Sender: TObject);
var
  stNodeNo : string;
  stEcuID : string;
  stDeviceID: string;
