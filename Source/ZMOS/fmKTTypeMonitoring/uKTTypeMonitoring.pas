...
type
  ...
  public
    ...
    ZoneFormDisplayList : TStringList;
...
//    procedure ShowMessage(aMessage:string);
    ...
    procedure DisplayZoneMapShow(aBuildingCode,aNodeNo,aECUID,aPortNo: string);
    procedure DisplayZoneMapModeChange(aBuildingCode,aNodeNo,aECUID,aPortNo: string);
  public
...
begin
  ...
  if ZoneFormDisplayList.Count > 0 then
  begin
    for i := ZoneFormDisplayList.Count - 1 downto 0 do  TfmZoneMap(ZoneFormDisplayList.Objects[i]).Free;
  end;
  ZoneFormDisplayList.Clear;
  MemoryDestory;
...
{  //위치 정보 보기의 Map 에 알람 표시
  ...
  begin
    ...
    begin
      ...
      end else if (nAlarmSound > 0) and (G_nZoneMapPopup = 1) then
      begin
        stTempBuilding := dmDBFunction.GetTB_ZONEDEVICE_BuildingCode(aNodeNo,stECUID,stPortNo);
        DisplayZoneMapShow(copy(stTempBuilding,1,9),aNodeNo,stECUID,stPortNo);
      end;
  ...
  end else if (G_nZoneMapPopup = 1) then
  begin
    stTempBuilding := dmDBFunction.GetTB_ZONEDEVICE_BuildingCode(aNodeNo,stECUID,stPortNo);
    DisplayZoneMapModeChange(copy(stTempBuilding,1,9),aNodeNo,stECUID,stPortNo);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        stTempBuilding := dmDBFunction.GetTB_ZONEDEVICE_BuildingCode(inttostr(NodeNo),ECUID,aPortNo);
        DisplayZoneMapShow(copy(stTempBuilding,1,9),inttostr(NodeNo),ECUID,aPortNo);
      end;
    end;
  end;
  ...
  begin
    stTempBuilding := dmDBFunction.GetTB_ZONEDEVICE_BuildingCode(inttostr(NodeNo),ECUID,aPortNo);
    DisplayZoneMapModeChange(copy(stTempBuilding,1,9),inttostr(NodeNo),ECUID,aPortNo);
  end;
...
begin
  ...
  ZoneFormDisplayList := TStringList.Create;
...
procedure TfmKTTypeMonitoring.DisplayZoneMapModeChange(aBuildingCode,
  aNodeNo, aECUID, aPortNo: string);
var
  nIndex : integer;
begin
  nIndex := ZoneFormDisplayList.IndexOf(aBuildingCode);
  if nIndex > -1 then
  begin
    TfmZoneMap(ZoneFormDisplayList.Objects[nIndex]).Add_AlarmArea(aNodeNo,aEcuID,aPortNo);
    //TfmAlarmMap(AlarmFormDisplayList.Objects[nIndex]).Show;
  end;
end;

procedure TfmKTTypeMonitoring.DisplayZoneMapShow(aBuildingCode, aNodeNo,
  aECUID, aPortno: string);
var
  nIndex : integer;
  oDisplayForm : TfmZoneMap;
begin
  nIndex := ZoneFormDisplayList.IndexOf(aBuildingCode);
  if nIndex < 0 then
  begin
    oDisplayForm := TfmZoneMap.Create(Self);
    oDisplayForm.BuildingCode := aBuildingCode;
    oDisplayForm.Show;
    oDisplayForm.Add_AlarmArea(aNodeNo,aEcuID,aPortno);
    ZoneFormDisplayList.AddObject(aBuildingCode,oDisplayForm);
  end else
  begin
    TfmZoneMap(ZoneFormDisplayList.Objects[nIndex]).Add_AlarmArea(aNodeNo,aEcuID,aPortno);
    TfmZoneMap(ZoneFormDisplayList.Objects[nIndex]).Show;
  end;
end;

procedure TfmKTTypeMonitoring.CommandArrayCommandsTZONEMAPCLOSEExecute(
  Command: TCommand; Params: TStringList);
var
  stBuildingCode : string;
  nIndex : integer;
begin
  inherited;
  stBuildingCode := Params.Values['Data'];
  nIndex := ZoneFormDisplayList.IndexOf(stBuildingCode);
  if nIndex > -1 then
  begin
