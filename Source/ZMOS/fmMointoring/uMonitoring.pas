...
type
  TfmMonitoring = class(TfmASubForm)
    ...
    procedure doorListViewMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
...
begin
  ...
  self.FindSubForm('Main').FindCommand('FORMSHOW').Params.Values['VALUE'] := 'Monitoring';
...
begin
  ...
  end else if DoorList.ViewStyle =  vsIcon then
  begin
    DoorList.ViewStyle := vsList;
    DoorList.Refresh;
    DoorList.ViewStyle := vsIcon;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AlarmList.Items[I].ImageIndex:= JaeJungAlarmStateImageIndex;
        end else
        begin
      ...
      begin
        ...
        begin
          ...
          InsertDOORList(stPermitCode,stPermit,stTime,stLocationName,stDoorName,stCardNo,stUserName,
            stCompanyName,stJijumName,'',stEmID,stDeviceID,stHandPhone,stCompanyPhone,stAddr1,stAddr2,sg_alarmAccess);
          Exit;
        end;
      end;
    end;
    if stPermitCode <> '1' then   //출입승인이 아니면 방범 구역도 보자
    begin
    //if stDoorNo = '0' then
    //begin
      ...
      stDoorName:= AlarmListView.Items[nIndex].Caption; // 경계구역 명
      InsertDOORList(stPermitCode,stPermit,stTime,stLocationName,stDoorName,stCardNo,stUserName,
      stCompanyName,stJijumName,'',stEmID,stDeviceID,stHandPhone,stCompanyPhone,stAddr1,stAddr2,sg_alarmAccess);
      Exit;
    end else Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      Cells[AlarmDoorIndexArray[11],1] := aDoorID; //출입문아이디
      Cells[AlarmDoorIndexArray[12],1] := aHandPhone; //핸드폰
      Cells[AlarmDoorIndexArray[13],1] := aCompanyPhone; //사내전화번호
      Cells[AlarmDoorIndexArray[14],1] := aAddr1; //주소1
      Cells[AlarmDoorIndexArray[15],1] := aAddr2; //주소2
...
//  stMCUID := copy(aReceiveData,8,7);
  ...
  nIndex := AlarmCodeList.IndexOf(FillZeroNumber(strtoint(aNodeNo),3) + stECUID + stZoneCode);
  ...
  end;  }
{
  ...
  begin
    ...
    begin
      ...
      if stSubCLass <> 'EL' then stPortName := GetPortName(aNodeNo,stECUID,stPortNo)
      else stPortName := dmDBFunction.GetDeviceName(aNodeNo,stECUID) + '_' + stSubAddr;
  ...
  stDeviceType := dmDBFunction.GetDeviceType(stSubCLass) + '[' + stSubAddr + ']';
...
//  GridSetting; //그리드를 사용자 환경에 맞게 셋팅함
  ...
  pm_initPosiNega.Visible := G_bPosiNega;
...
begin
  ...
  begin
    stAlarmName := AlarmListView.items[nIndex].Caption;
    Try
      Try
...
begin
  ...
  stDeviceID := copy(stAlarmID,1,5);
  nDviceIndex := DeviceStateList.IndexOf(stDeviceID);
  if nDviceIndex < 0 then Exit;
  if ALARMVIEW = 'QUERY' then
  begin
    stAlarmName := IntAlarmListView.items[nIndex].Caption;
    Try
      Try
        fmAlarmConfirm:= TfmAlarmConfirm.Create(Self);
      Except
    ...
    if bAlarmConfirm then
    begin
      TDeviceState(DeviceStateList.Objects[nDviceIndex]).AlaramEventClear;
      AlarmListClear;
      //TDeviceState(DeviceStateList.Objects[nAlarmIndex]).AlaramEventClear;
      self.FindSubForm('Main').FindCommand('SendData').Params.Values['VALUE'] := 'ALARMREFRESH'+ DATADELIMITER + '00000' + DATADELIMITER;
      self.FindSubForm('Main').FindCommand('SendData').Execute;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FindField('AL_ALARMDEVICETYPECODE').AsString <> 'EL' then
            stPortName := GetPortName(inttostr(FindField('AC_NodeNo').asinteger),FindField('AC_ECUID').AsString,FindField('AL_ZONENO').AsString )
          else stPortName := dmDBFunction.GetDeviceName(inttostr(FindField('AC_NodeNo').asinteger),FindField('AC_ECUID').AsString) + '_' + FindField('AL_SUBADDR').AsString;
...
begin
  ...
  begin
    DoorListView.Items[nDoorIndex].ImageIndex:= DoorFireStateImageIndex;
    if PatrolUse then DoorListView.Items[nDoorIndex].ImageIndex:= DoorFireStateImageIndex;
  end else
  begin
  ...
  begin
    ...
    begin
      ...
      if PatrolUse then IntdoorListView.Items[nDoorIndex].StateIndex:= DoorMasterModeStateIndex;
    end else
    begin
      DoorListView.Items[nDoorIndex].StateIndex := DoorOpenModeStateIndex;
      if PatrolUse then IntdoorListView.Items[nDoorIndex].StateIndex:= DoorOpenModeStateIndex;
...
procedure TfmMonitoring.AlarmModeChanged(NodeNo: integer; aEcuID,aAlarmArea: string;
  aWachMode: TWatchMode);
var
  nAlarmIndex : integer;
begin
  nAlarmIndex := AlarmCodeList.IndexOf(FillzeroNumber(NodeNo,3) + aEcuID + aAlarmArea);
  ...
  case aWachMode of
    ...
    cmJaejung :
      begin
        AlarmListView.Items[nAlarmIndex].ImageIndex:= JaeJungAlarmStateImageIndex;
...
var
  nAlarmIndex : integer;
begin
  nAlarmIndex := AlarmCodeList.IndexOf(FillzeroNumber(NodeNo,3) + ECUID + inttostr(strtoint(aArmAreaNo)) );
...
procedure TfmMonitoring.AlarmEventProcess(NodeNo: integer; ECUID,aArmAreaNo, aCmd,
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if nDoorIndex > -1 then
        begin
          stDoorName:= doorListView.Items[nDoorIndex].Caption + '_닫힘'; // 경계구역 명
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          InsertDOORList(aPermit,stPermitName,aTime,stBuildingName,stDoorName,aCardNo,stEmName,
            stCompanyName,stJijumName,stDepartName,stEmCode,stDoorID,stHandPhone,stCompanyPhone,stAddr1,stAddr2,sg_alarmAccess);
          Exit;
        end;
      end;
    end;
    ...
    //if aDoorNo = '0' then
    //begin
      nDoorIndex := AlarmCodeList.IndexOf(stDeviceID + aArmAreaNo);

      if nDoorIndex < 0 then Exit;// 관제 권한이 없음
      stDoorName:= AlarmListView.Items[nDoorIndex].Caption; // 경계구역 명
      InsertDOORList(aPermit,stPermitName,aTime,stBuildingName,stDoorName,aCardNo,stEmName,
        stCompanyName,stJijumName,stDepartName,stEmCode,stDoorID,stHandPhone,stCompanyPhone,stAddr1,stAddr2,sg_alarmAccess);
      InsertDOORList(aPermit,stPermitName,aTime,stBuildingName,stDoorName,aCardNo,stEmName,
...
procedure TfmMonitoring.MenuItem8Click(Sender: TObject);
var
  stCardNo : string;
begin
  stCardNo := sg_Access.Cells[DoorIndexArray[5],sg_Access.Row];
...
procedure TfmMonitoring.MenuItem10Click(Sender: TObject);
var
  stCardNo : string;
begin
...
procedure TfmMonitoring.MenuItem9Click(Sender: TObject);
var
  stCardNo : string;
begin
  stCardNo := sg_Access.Cells[DoorIndexArray[5],sg_Access.Row];
...
procedure TfmMonitoring.MenuItem14Click(Sender: TObject);
var
  stCardNo : string;
begin
  stCardNo := sg_IntAccess.Cells[IntDoorIndexArray[5],sg_IntAccess.Row];
...
procedure TfmMonitoring.iniConfigWrite;
var
  ini_fun : TiniFile;
  i : integer;
begin
  ...
  begin
    ini_fun.WriteInteger('Monitoring','DoorIndexArray'+inttostr(i),DoorIndexArray[i]);
  end;
  for i := LOW(relDoorIndexArray) to HIGH(relDoorIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','relDoorIndexArray'+inttostr(i),relDoorIndexArray[i]);
  end;
  for i := LOW(IntDoorIndexArray) to HIGH(IntDoorIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','IntDoorIndexArray'+inttostr(i),IntDoorIndexArray[i]);
  end;
  for i := LOW(relIntDoorIndexArray) to HIGH(relIntDoorIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','relIntDoorIndexArray'+inttostr(i),relIntDoorIndexArray[i]);
  end;
  for i := LOW(AlarmDoorIndexArray) to HIGH(AlarmDoorIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','AlarmDoorIndexArray'+inttostr(i),AlarmDoorIndexArray[i]);
  end;
  for i := LOW(relAlarmDoorIndexArray) to HIGH(relAlarmDoorIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','relAlarmDoorIndexArray'+inttostr(i),relAlarmDoorIndexArray[i]);
  end;
  for i := LOW(AlarmIndexArray) to HIGH(AlarmIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','AlarmIndexArray'+inttostr(i),AlarmIndexArray[i]);
  end;
  for i := LOW(relAlarmIndexArray) to HIGH(relAlarmIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','relAlarmIndexArray'+inttostr(i),relAlarmIndexArray[i]);
  end;
  for i := LOW(IntAlarmIndexArray) to HIGH(IntAlarmIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','IntAlarmIndexArray'+inttostr(i),IntAlarmIndexArray[i]);
  end;
  for i := LOW(relIntAlarmIndexArray) to HIGH(relIntAlarmIndexArray) do
  begin
    ini_fun.WriteInteger('Monitoring','relIntAlarmIndexArray'+inttostr(i),relIntAlarmIndexArray[i]);
  end;
  ...
  for i := 0 to ACCESSEVENTMAXCELL do
  begin
    Access3SizeTable[i] := sg_alarmAccess.ColWidths[i];
    ini_fun.WriteInteger('출입이벤트3사이즈',inttostr(i),sg_alarmAccess.ColWidths[i]);
  end;
  for i := 0 to ALARMEVENTMAXCELL do
  begin
    Alarm1SizeTable[i] := sg_Alarm.ColWidths[i];
    ini_fun.WriteInteger('알람이벤트1사이즈',inttostr(i),sg_Alarm.ColWidths[i]);
  end;
  for i := 0 to ALARMEVENTMAXCELL do
  begin
...
procedure TfmMonitoring.iniConfigRead;
var
  ini_fun : TiniFile;
  i : integer;
  stValue : string;
begin
  ...
  for i := LOW(DoorIndexArray) to HIGH(DoorIndexArray) do
  begin
    DoorIndexArray[i] := ini_fun.ReadInteger('Monitoring','DoorIndexArray'+inttostr(i),i);
  end;
  for i := LOW(relDoorIndexArray) to HIGH(relDoorIndexArray) do
  begin
    relDoorIndexArray[i] := ini_fun.ReadInteger('Monitoring','relDoorIndexArray'+inttostr(i),i);
  end;
  for i := LOW(IntDoorIndexArray) to HIGH(IntDoorIndexArray) do
  begin
    IntDoorIndexArray[i] := ini_fun.ReadInteger('Monitoring','IntDoorIndexArray'+inttostr(i),i);
  end;
  for i := LOW(relIntDoorIndexArray) to HIGH(relIntDoorIndexArray) do
  begin
    relIntDoorIndexArray[i] := ini_fun.ReadInteger('Monitoring','relIntDoorIndexArray'+inttostr(i),i);
  end;
  for i := LOW(AlarmDoorIndexArray) to HIGH(AlarmDoorIndexArray) do
  begin
    AlarmDoorIndexArray[i] := ini_fun.ReadInteger('Monitoring','AlarmDoorIndexArray'+inttostr(i),i);
  end;
  for i := LOW(relAlarmDoorIndexArray) to HIGH(relAlarmDoorIndexArray) do
  begin
    relAlarmDoorIndexArray[i] := ini_fun.ReadInteger('Monitoring','relAlarmDoorIndexArray'+inttostr(i),i);
  end;
  for i := LOW(AlarmIndexArray) to HIGH(AlarmIndexArray) do
  begin
    AlarmIndexArray[i] := ini_fun.ReadInteger('Monitoring','AlarmIndexArray'+inttostr(i),i);
  end;
  for i := LOW(relAlarmIndexArray) to HIGH(relAlarmIndexArray) do
  begin
    relAlarmIndexArray[i] := ini_fun.ReadInteger('Monitoring','relAlarmIndexArray'+inttostr(i),i);
  end;
  for i := LOW(IntAlarmIndexArray) to HIGH(IntAlarmIndexArray) do
  begin
    IntAlarmIndexArray[i] := ini_fun.ReadInteger('Monitoring','IntAlarmIndexArray'+inttostr(i),i);
  end;
  for i := LOW(relIntAlarmIndexArray) to HIGH(relIntAlarmIndexArray) do
  begin
    relIntAlarmIndexArray[i] := ini_fun.ReadInteger('Monitoring','relIntAlarmIndexArray'+inttostr(i),i);
  end;
...
procedure TfmMonitoring.MemoryCreate;
begin
...
procedure TfmMonitoring.ListHeaderNameSet;
var
  i : integer;
begin
  for i := LOW(DoorIndexArray) to HIGH(DoorIndexArray) do
  ...
  for i := LOW(AlarmDoorIndexArray) to HIGH(AlarmDoorIndexArray) do
  begin
    sg_AlarmAccess.cells[AlarmDoorIndexArray[i],0] := AccessHeaderNameList[i];
  end;
...
procedure TfmMonitoring.CellHide;
var
  i : integer;
begin
  for i := 0 to ACCESSEVENTMAXCELL do
  begin
    ...
    begin
      ...
      sg_AlarmAccess.ColWidths[AlarmDoorIndexArray[i]] := 0;
    end else
    begin
      ...
      sg_AlarmAccess.ColWidths[AlarmDoorIndexArray[i]] := Access3SizeTable[AlarmDoorIndexArray[i]];
      if sg_AlarmAccess.ColWidths[AlarmDoorIndexArray[i]] = 0 then sg_AlarmAccess.ColWidths[AlarmDoorIndexArray[i]] := 90;
    end;
  end;
  for i := 0 to ALARMEVENTMAXCELL do
  begin
...
procedure TfmMonitoring.MemoryDestory;
begin
...
procedure TfmMonitoring.mn_IntFireRecoveryClick(Sender: TObject);
var
  stAlarmID : string;
  i : integer;
begin
...
procedure TfmMonitoring.mn_FireRecoveryClick(Sender: TObject);
var
  stAlarmID : string;
  i : integer;
begin
...
procedure TfmMonitoring.pm_JavaraCloseClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.pm_intiJavarOpenClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.pm_intiJavaraCloseClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.doorListViewMouseMove(Sender: TObject;
 ...
 {  if Assigned(AItem) then
  begin
    ...
    begin
      ...
      TListView(Sender).Selected := AItem;
    end;
  end else exit;  }
...
procedure TfmMonitoring.doorListViewSelectItem(Sender: TObject;
...
procedure TfmMonitoring.pm_MasterModeClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.pm_IntiMasterModeClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.PositiveClick(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
...
procedure TfmMonitoring.Negaitve1Click(Sender: TObject);
var
  stDoorID : string;
  i : integer;
begin
