...
//    procedure LoadsPosiCode(aCompanyCode:string;cmb_Box:TComboBox);
    ...
    Function InsertTB_CARD(aCardNo,aCardGubun,aCardType,aEmpID,aCompanyCode,aStick:string) : Boolean;
    Function UpdateTB_CARD(aCardNo,aCardGubun,aCardType,aEmpID,aCompanyCode,aStick:string) : Boolean;
...
uses uDataModule1,
     ...
     DIMime,
     uSHFingerDeviceReg, uDBFunction, UCommonModule;
...
begin
  ...
  begin
    ...
    if cmb_sCompany.ItemIndex > 0 then
    begin
      LoadsPosiCode(cells[EmployeeIndexArray[6],Row],sPosiCodeList,cmb_sPosi);
    ...
    nIndex := sEmpTypeCodeList.IndexOf(cells[EmployeeIndexArray[21],Row]);
    if nIndex > -1 then cmb_emType.ItemIndex := nIndex;

    nIndex := sWorkGubunCodeList.IndexOF(cells[EmployeeIndexArray[31],Row]);
    ...
    if(cells[EmployeeIndexArray[33],Row] = 'Y') then chk_Stick.Checked := True
    else chk_Stick.Checked := False;
    ...
    begin
      ...
      if cells[EmployeeIndexArray[20],Row] = copy(cmb_RegGubun.Items[i],1,1) then
      begin
        cmb_RegGubun.ItemIndex := i;
        break;
      end;
    end;
    ed_CardNo.Text := Trim(cells[EmployeeIndexArray[19],Row]);
    ed_fdmsNo.Text := cells[EmployeeIndexArray[22],Row];
    L_stOldFdmsNo := cells[EmployeeIndexArray[22],Row];

    if DataModule1.DupCheckTB_CARDFINGER(cells[EmployeeIndexArray[22],Row],'1') then lb_FingerState.Caption := '등록'
    else lb_FingerState.Caption := '미등록';
    

    if cells[EmployeeIndexArray[24],Row] = '0' then rg_TimeGroup1.Checked := True
    else if cells[EmployeeIndexArray[24],Row] = '1' then rg_TimeGroup2.Checked := True;

    if cells[EmployeeIndexArray[23],Row] <> 'Y' then rg_TimeCodeNotUse.Checked := True;
    if cells[EmployeeIndexArray[25],Row] = 'Y' then chk_Time1.Checked := True
    else chk_Time1.Checked := False;
    if cells[EmployeeIndexArray[26],Row] = 'Y' then chk_Time2.Checked := True
    else chk_Time2.Checked := False;
    if cells[EmployeeIndexArray[27],Row] = 'Y' then chk_Time3.Checked := True
    else chk_Time3.Checked := False;
    if cells[EmployeeIndexArray[28],Row] = 'Y' then chk_Time4.Checked := True
    else chk_Time4.Checked := False;
    stTemp := cells[EmployeeIndexArray[29],Row];
    if stTemp <> '' then
    begin
    ...
    stTemp := cells[EmployeeIndexArray[30],Row];
    if stTemp = 'Y' then chk_Master.Checked := True
    else chk_Master.Checked := False;
    ed_CardNumber.Text := cells[EmployeeIndexArray[32],Row];
...
begin
  ...
  if dmDBFunction.CheckTB_CARDFromEmployeeID(stCompanyCode,ed_sEmpNo.Text) then
  begin
  ...
  //DeleteTB_FingerDeviceCard_FingerNo(sg_Employ.cells[EmployeeIndexArray[22],sg_Employ.Row]);
  //DeleteTB_CardFinger_FingerNo(sg_Employ.cells[EmployeeIndexArray[22],sg_Employ.Row]);
...
begin
  ...
  begin
    //if fdmsNoAuto then stFdmsId := DataModule1.GetNextFdmsID //Insert 버튼 클릭 할때 자동으로 생성 됨
    //else
    //begin
...
begin
  ...
  begin
    ...
    begin
      if stMode = 'Insert' then  bResult := InsertTB_CARD(aCardNo,'1',aRegGubun,aEmpID,aCompanyCode,'Y')
      else if stMode = 'Update' then bResult := UpdateTB_CARD(aCardNo,'1',aRegGubun,aEmpID,aCompanyCode,'Y');
...
//    UpdateTB_FoodEventCARDNO(L_stOldCardNo,aCardNo);
  ...
  begin
    ...
    if bResult then //카드번호 입력하자.
    begin
      if chk_Stick.Checked then stStick := 'Y'
      else stStick := 'N';
      if stMode = 'Insert' then  bResult := InsertTB_CARD(aCardNo,'1',aRegGubun,aEmpID,aCompanyCode,'Y')
      else if stMode = 'Update' then bResult := UpdateTB_CARD(aCardNo,'1',aRegGubun,aEmpID,aCompanyCode,stStick);
  ...
  if Trim(L_stOldCardNo) <> Trim(aCardNo) then
    dmDBFunction.UpdateTB_CARD_Field_StringValue(aCardNo,'CA_STICK','Y');
...
function TfmEmploy.InsertTB_CARD(aCardNo, aCardGubun, aCardType, aEmpID,
  aCompanyCode,aStick: string): Boolean;
var
  stSql : string;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Cells[EmployeeIndexArray[19],nRow] := FindField('CA_CARDNO').AsString;
          Cells[EmployeeIndexArray[20],nRow] := FindField('CA_CARDTYPE').AsString;
          Cells[EmployeeIndexArray[21],nRow] := FindField('RG_CODE').AsString;
          Cells[EmployeeIndexArray[22],nRow] := FindField('FDMS_ID').AsString;
          Cells[EmployeeIndexArray[23],nRow] := FindField('DE_TIMECODEUSE').AsString;
          Cells[EmployeeIndexArray[24],nRow] := FindField('TC_GROUP').AsString;
          Cells[EmployeeIndexArray[25],nRow] := FindField('TC_TIME1').AsString;
          Cells[EmployeeIndexArray[26],nRow] := FindField('TC_TIME2').AsString;
          Cells[EmployeeIndexArray[27],nRow] := FindField('TC_TIME3').AsString;
          Cells[EmployeeIndexArray[28],nRow] := FindField('TC_TIME4').AsString;
          Cells[EmployeeIndexArray[29],nRow] := FindField('TC_WEEKCODE').AsString;
          Cells[EmployeeIndexArray[30],nRow] := FindField('EM_MASTER').AsString;
          Cells[EmployeeIndexArray[31],nRow] := FindField('WG_CODE').AsString;
          Cells[EmployeeIndexArray[32],nRow] := FindField('POSITIONNUM').AsString;
          Cells[EmployeeIndexArray[33],nRow] := FindField('CA_STICK').AsString;
...
var
  ...
  i : integer;
begin
  ini_fun := TiniFile.Create(ExeFolder + '\EmployeeReport.INI');
...
begin
  ...
  for i := LOW(relEmployeeIndexArray) to HIGH(relEmployeeIndexArray) do
  begin
    ini_fun.WriteInteger('Employee','relEmployeeIndexArray'+inttostr(i),relEmployeeIndexArray[i]);
  end;
...
procedure TfmEmploy.SetShowCount(const Value: integer);
begin
  FShowCount := Value;
  lb_Count.Caption := inttostr(Value) + ' 건';
end;

procedure TfmEmploy.sg_EmployColumnMoved(Sender: TObject; FromIndex,
  ToIndex: Integer);
begin
  inherited;
...
procedure TfmEmploy.ChangeEmployeeIndex(FromIndex, ToIndex: integer);
var
  i : integer;
  nChangData : integer;
begin
...
procedure TfmEmploy.GridSetting;
var
  i : integer;
begin
  for i:=0 to sg_Employ.ColCount - 1 do
...
procedure TfmEmploy.MemoryCreate;
begin
...
procedure TfmEmploy.ListHeaderNameSet;
var
  i : integer;
begin
  for i := LOW(EmployeeIndexArray) to HIGH(EmployeeIndexArray) do
...
procedure TfmEmploy.EmployeeHeaderNameCreate;
begin
  ...
  EmployeeHeaderNameList.Add('카드번호');     //19
  EmployeeHeaderNameList.Add('카드등록구분'); //20
  EmployeeHeaderNameList.Add('직원구분코드'); //21
  EmployeeHeaderNameList.Add('지문번호');     //22
  EmployeeHeaderNameList.Add('타임코드사용유무'); //23
  EmployeeHeaderNameList.Add('타임그룹');     //24
  EmployeeHeaderNameList.Add('타임1');        //25
  EmployeeHeaderNameList.Add('타임2');        //26
  EmployeeHeaderNameList.Add('타임3');        //27
  EmployeeHeaderNameList.Add('타임4');        //28
  EmployeeHeaderNameList.Add('주간');         //29
  EmployeeHeaderNameList.Add('마스터카드');   //30
  EmployeeHeaderNameList.Add('재직구분');   //31
  EmployeeHeaderNameList.Add('관제등록번호'); //32
  EmployeeHeaderNameList.Add('수동등록'); //33
