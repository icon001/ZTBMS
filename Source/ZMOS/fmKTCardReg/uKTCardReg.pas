...
uses
  ...
  LMDFileCtrl,ActiveX,ADODB,WinSpool, DB,IniFiles,uMain,Imm, AdvObj;
...
type
  TfmKTCardReg = class(TfmASubForm)
    Panel12: TPanel;
    Panel2: TPanel;
    ...
    procedure ed_sEmpNoEnter(Sender: TObject);
  ...
  private
    ...
    procedure FormEnable(aValue:Boolean);
  ...
  private
    Function GetCheckOldFormat(aEmpCode:string):Boolean;
    Function GetCheckNewChangeTable(aOldEmpCode:string;var aNewEmpCode:string):Boolean;
  public
    { Public declarations }
...
procedure TfmKTCardReg.FormShow(Sender: TObject);
var
  nIndex : integer;
  ini_fun : TiniFile;
...
procedure TfmKTCardReg.Panel2Resize(Sender: TObject);
var
  nWidth : integer;
  nBlock : integer;
  nLeft : integer;
...
procedure TfmKTCardReg.btn_CloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfmKTCardReg.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  self.FindSubForm('Main').FindCommand('FORMSHOW').Params.Values['VALUE'] := 'KTCardReg';
...
procedure TfmKTCardReg.ed_sEmpNoExit(Sender: TObject);
begin
  L_stReadCardNo := '';
  EmployeeSearch(btn_CardRead.Enabled);
end;

procedure TfmKTCardReg.FormCreate(Sender: TObject);
begin
...
procedure TfmKTCardReg.FormNameSet;
begin
...
procedure TfmKTCardReg.btn_SearchClick(Sender: TObject);
begin
  ShowEmployee('');
end;

procedure TfmKTCardReg.cmb_CompanyChange(Sender: TObject);
begin
  LoadJijumCode(CompanyCodeList.Strings[cmb_Company.ItemIndex],JijumCodeList,cmb_Jijum);
...
procedure TfmKTCardReg.cmb_JijumChange(Sender: TObject);
begin
  LoadDepartCode(copy(JijumCodeList.Strings[cmb_Jijum.ItemIndex],1,3),copy(JijumCodeList.Strings[cmb_Jijum.ItemIndex],4,3),DepartCodeList,cmb_Depart);
//  btn_SearchClick(Self);
end;

procedure TfmKTCardReg.cmb_DepartChange(Sender: TObject);
begin
//  btn_SearchClick(Self);

end;

procedure TfmKTCardReg.cmb_PosiChange(Sender: TObject);
begin
//  btn_SearchClick(Self);
end;

procedure TfmKTCardReg.cmb_sCompanyChange(Sender: TObject);
begin
...
procedure TfmKTCardReg.cmb_sJijumChange(Sender: TObject);
begin
  LoadsDepartCode(copy(sJijumCodeList.Strings[cmb_sJijum.ItemIndex],1,3),copy(sJijumCodeList.Strings[cmb_sJijum.ItemIndex],4,3),sDepartCodeList,cmb_sDepart);

end;

procedure TfmKTCardReg.btn_ZipcodeSearchClick(Sender: TObject);
begin
...
procedure TfmKTCardReg.CommandArrayCommandsTCommand0Execute(
...
procedure TfmKTCardReg.btn_ImageClick(Sender: TObject);
begin
...
procedure TfmKTCardReg.ShowEmployee(aCode: string; aTopRow: integer);
var
  stSql : string;
  nRow : integer;
  i : integer;
...
procedure TfmKTCardReg.btn_PortRefreshClick(Sender: TObject);
var
  nCount : integer;
  i : integer;
  stTemp : string;
...
function TfmKTCardReg.DecodeCommportName(PortName: String): WORD;
var
 Pt : Integer;
begin
...
function TfmKTCardReg.EncodeCommportName(PortNum: WORD): String;
begin
 if PortNum < 10
...
function TfmKTCardReg.GetSerialPortList(List: TStringList;
...
function TfmKTCardReg.CheckEmployee(aEmCode: string): Boolean;
var
  stSql : string;
  nIndex : integer;
...
begin
  ...
  stSql := stSql + ' AND b.CA_GUBUN = ''1'' ) ';  //일반카드
...
function TfmKTCardReg.GetCardNextSeq(aEmCode: string): string;
var
  stSql : string;
begin
  result := '051';
  ...
  begin
    ...
    result := FillZeroNumber((FindField('CARD_SEQ').AsInteger + 1),3);
  end;
end;

procedure TfmKTCardReg.ed_sEmpNoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfmKTCardReg.btn_CancelClick(Sender: TObject);
begin
  FormClear;
end;

procedure TfmKTCardReg.FormClear(aFull:Boolean=True);
var
  nIndex : integer;
begin
  if aFull then
...
procedure TfmKTCardReg.btn_UpdateClick(Sender: TObject);
var
  stCardNo : string;
  stCompanyCode : string;
  stJijumCode : string;
...
begin
  ...
  if nResult = 0  then
  begin
    showmessage('발급 되지 않은 카드입니다. ' + fmMain.mn_KTCardCreate.Caption + ' 메뉴를 이용하여 카드발급 후 사용하세요.');
    Exit;
  end;
...
function TfmKTCardReg.InialzeCard: Boolean;
var
  stSendData : string;
  PastTime : dword;
begin
...
function TfmKTCardReg.SearchCardInfo(aCardNo: string;aCheck:Boolean=True): integer;
var
  stSendData : string;
  PastTime : dword;
  stCardNo : string;
...
begin
  ...
  begin
    ...
    begin
      ...
      if GetCheckOldFormat(copy(stCardNo,1,9)) then
      begin
        //기존 테이블에 사원번호가 존재 하면....
        ed_EmpNo.Text := copy(stCardNo,1,9);
        ed_sEmpNo.Text := copy(stCardNo,1,9);
        ed_CardSeq.Text := copy(stCardNo,10,2);
      end else if GetCheckOldFormat(copy(stCardNo,2,8)) then  //신 사원증이면
      begin
        //기존 테이블에 신사원번호가 존재 하면....
        ed_EmpNo.Text := copy(stCardNo,2,8);
        ed_sEmpNo.Text := copy(stCardNo,2,8);
        ed_CardSeq.Text := copy(stCardNo,10,2);
      end else if GetCheckNewChangeTable(copy(stCardNo,1,9),stNewEmpCode) then
      begin
        //변경된 테이블에 있는 사원번호인지 체크
        ed_EmpNo.Text := '3' + stNewEmpCode;
        ed_sEmpNo.Text := '3' + stNewEmpCode;
        ed_CardSeq.Text := copy(stCardNo,10,2);
      end else
      begin
        ed_EmpNo.Text := copy(stCardNo,1,9);
...
function TfmKTCardReg.WriteCardInfo(aCardNo: string): Boolean;
var
  stSendData : string;
  PastTime : dword;
begin
...
procedure TfmKTCardReg.ApdDataPacket1StringPacket(Sender: TObject;
  Data: String);
begin
  RcvCardDataByReader(Data);
  //fmMain.ApdDataPacket2.Enabled := True;
end;

procedure TfmKTCardReg.RcvCardDataByReader(aData: String);
var
  nPos : integer;
begin
...
function TfmKTCardReg.CopyGradeOldCard(aOldCard,
  aNewCard: string): Boolean;
begin
  result := InsertTB_DEVICECARDNONotExist(aOldCard,aNewCard);
end;

function TfmKTCardReg.CheckTB_CARDCardNo(aCardNo: string): Boolean;
var
  stSql : string;
begin
  result := False;
...
function TfmKTCardReg.DeleteTB_DEVICECARDNOCardNo(
...
function TfmKTCardReg.InsertTB_DEVICECARDNONotExist(aOldCardNO,
...
function TfmKTCardReg.UpdateTB_DEVICECARDNOPermitDelete(
...
function TfmKTCardReg.DeleteTB_CARDCardNo(aOldCardNO: string): Boolean;
var
  stSql :string;
begin
...
function TfmKTCardReg.EmployeeInfoSave: Boolean;
var
  stCompanyCode : string;
  stJijumCode : string;
  stDepartCode : string;
...
function TfmKTCardReg.DupCheckTB_Employee(aCompanyCode,
...
function TfmKTCardReg.InsertTB_EMPLOYEE(aEmpID, aEmpNM, aCompanyCode,
...
begin
  ...
  result := DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_EMPLOYEECHANGE(aCompanyCode,aEmpID,aCardNo,'KTCardReg','1');
  DataModule1.ProcessExecSQL(stSql);

end;

function TfmKTCardReg.UpdateTB_EMPLOYEE(aEmpID, aEmpNM, aCompanyCode,
  aJijumCode, aDepartCode, aPosiCod, aCompanyPhone, aJoinDate, aRetireDate,
  aZipcode, aAddr1, aAddr2, aHomePhone, aHandphone, aRegGubun, aCardNo,
...
begin
  ...
  result := DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_EMPLOYEECHANGE(aCompanyCode,aEmpID,aCardNo,'KTCardReg','2');
  DataModule1.ProcessExecSQL(stSql);
...
function TfmKTCardReg.InsertTB_CARD(aCardNo, aCardGubun, aCardType,
...
function TfmKTCardReg.UpdateTB_CARD(aCardNo, aCardGubun, aCardType,
  aEmpID, aCompanyCode: string): Boolean;
var
  stSql : string;
begin
...
function TfmKTCardReg.GetEmployeeFdmsID(aCompanyCode,
...
function TfmKTCardReg.UpdateTB_DEVICECARDNO(aCardNo,
...
function TfmKTCardReg.GetMaxPositionNum: integer;
var
  stSql : string;
  TempAdoQuery : TADOQuery;
begin
...
procedure TfmKTCardReg.btn_CardReadClick(Sender: TObject);
begin
...
function TfmKTCardReg.SaveTB_KTCARDISSUE(aEmCode,
...
function TfmKTCardReg.CheckTB_KTCARDISSUE(aEmCode: string;var aCardSeq:string): Boolean;
var
  stSql : string;
begin
  aCardSeq := '';
...
function TfmKTCardReg.InsertTB_KTCARDISSUE(aEmCode,
...
function TfmKTCardReg.UpdateTB_KTCARDISSUE(aEmCode,
  aCardSeq,aWrite,aWriteDate: string): Boolean;
var
  stSql : string;
begin
  ...
  stSql := stSql + ' KI_WRITEDATE = ''' + aWriteDate + ''' ';   //등록 일자는 입력 하지 말자 발급일자만 확인 하기 위함
  stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND EM_CODE = ''' + aEmCode + '''';
...
procedure TfmKTCardReg.cmb_ComPortChange(Sender: TObject);
var
  ini_fun : TiniFile;
begin
...
function TfmKTCardReg.CardReadCheck(aData: string): Boolean;
var
  nPos : integer;
begin
  result := False;
...
function TfmKTCardReg.ACKCheck(aData: string): Boolean;
var
  nPos : integer;
begin
  result := False;
...
function TfmKTCardReg.NACKCheck(aData: string): Boolean;
var
  nPos : integer;
begin
  result := False;
...
procedure TfmKTCardReg.btn_employeesaveClick(Sender: TObject);
var
  stCardNo : string;
begin
  if cmb_sCompany.ItemIndex < 1 then
...
function TfmKTCardReg.EmployeeSearch(aClick: Boolean): string;
begin
  FormClear(False);
  ...
  if aClick then
  begin
    {if Length(ed_sEmpNo.Text) <> 8 then
    begin
      showmessage('사번은 8자리 입니다.');
      Exit;
    end;}
...
function TfmKTCardReg.ChangeCardNo(aOldCard, aNewCard: string): Boolean;
begin
...
function TfmKTCardReg.ChangeTB_ACCESSEVENTCardNo(aOldCard,
  aNewCard: string): Boolean;
var
  stSql : string;
begin
...
function TfmKTCardReg.ChangeTB_ATCARDCardno(aOldCard,
  aNewCard: string): Boolean;
var
  stSql : string;
begin
...
function TfmKTCardReg.UpdateTB_CARDCardNo(aOldCardNo,
  aNewCardNo: string): Boolean;
var
  stSql :string;
begin
...
procedure TfmKTCardReg.FormActivate(Sender: TObject);
begin
  inherited;
  self.FindSubForm('Main').FindCommand('FORMSHOW').Params.Values['VALUE'] := 'KTCardReg';
...
procedure TfmKTCardReg.FormEnable(aValue: Boolean);
begin
  ed_sEmpNM.Enabled := aValue;
  cmb_sCompany.Enabled := aValue;
  cmb_sJijum.Enabled := aValue;
  cmb_sDepart.Enabled := aValue;
  cmb_sPosi.Enabled := aValue;
  cmb_emType.Enabled := aValue;
  ed_sCompanyPhone.Enabled := aValue;
  dt_sJoinDt.Enabled := aValue;
  dt_sRetireDt.Enabled := aValue;
  ed_sZipcode.Enabled := aValue;
  ed_sAddr1.Enabled := aValue;
  ed_sAddr2.Enabled := aValue;
  ed_sHomePhone.Enabled := aValue;
  ed_sHandphone.Enabled := aValue;
  ed_CardNo.Enabled := aValue;
  ed_EmpImg.Enabled := aValue;
  ed_CardNo.Enabled := aValue;
  lb_FdmsID.Enabled := aValue;
  btn_employeesave.Enabled := aValue;

end;

procedure TfmKTCardReg.ed_sEmpNMEnter(Sender: TObject);
var
  TIMC: HIMC;
  dwSentence : DWORD;
  dwConversion : DWORD;
...
procedure TfmKTCardReg.ed_sEmpNoEnter(Sender: TObject);
var
  TIMC: HIMC;
  dwSentence : DWORD;
  dwConversion : DWORD;
...
function TfmKTCardReg.GetCheckOldFormat(aEmpCode: string): Boolean;
var
  stSql : string;
begin
  result := False;
...
function TfmKTCardReg.GetCheckNewChangeTable(aOldEmpCode: string;
  var aNewEmpCode: string): Boolean;
var
  stSql : string;
begin
  result := False;
  aNewEmpCode := '';
  stSql := ' Select * from tb_ktempchange ';
  stSql := stSql + ' Where oldempno = ''' + aOldEmpCode + ''' ';
  ...
  begin
    ...
    End;
    if RecordCount < 1 then Exit;
    aNewEmpCode := FindField('newempno').AsString;
    result := True;
  end;

end;

procedure TfmKTCardReg.CommandArrayCommandsTCloseExecute(Command: TCommand;
  Params: TStringList);
begin
  inherited;
  Close;
...
initialization
  RegisterClass(TfmKTCardReg);
Finalization
  UnRegisterClass(TfmKTCardReg);
