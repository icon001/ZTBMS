...
type
  TfmD2DFdState = class(TfmASubForm)
    GroupBox3: TGroupBox;
...
procedure TfmD2DFdState.btn_CloseClick(Sender: TObject);
begin
  Close;

end;

procedure TfmD2DFdState.btn_PrintClick(Sender: TObject);
var
...
begin
  ...
  stRefFileName  := ini_fun.ReadString('기간별식수통계','참조파일','D2DFDState.xls');
  stRefFileName := stPrintRefPath + stRefFileName;
  nExcelRowStart := ini_fun.ReadInteger('기간별식수통계','시작위치',6);
  ini_fun.Free;
...
procedure TfmD2DFdState.btn_FileSaveClick(Sender: TObject);
var
  aFileName: String;
  sDate: String;
  eDate: String;
...
procedure TfmD2DFdState.btn_SearchClick(Sender: TObject);
var
  stSql : string;
  i : integer;
  nRow : integer;
...
begin
  ...
  {
  if DBTYPE = 'MDB' then stSql := MDBSql.SelectTB_EMPLOYEEJoinD2DFoodState(FormatDateTime('yyyymmdd',dt_startDate.Date),FormatDateTime('yyyymmdd',dt_endDate.Date),stFoodArea,stFoodPermit)
  else if DBTYPE = 'PG' then stSql := PostGreSql.SelectTB_EMPLOYEEJoinD2DFoodState(FormatDateTime('yyyymmdd',dt_startDate.Date),FormatDateTime('yyyymmdd',dt_endDate.Date),stFoodArea,stFoodPermit)
  else if DBTYPE = 'MSSQL' then stSql := MSSql.SelectTB_EMPLOYEEJoinD2DFoodState(FormatDateTime('yyyymmdd',dt_startDate.Date),FormatDateTime('yyyymmdd',dt_endDate.Date),stFoodArea,stFoodPermit)
  else Exit; }

  if DBTYPE = 'MDB' then stSql := MDBSql.SelectTB_FOODEVENTD2DFoodState(FormatDateTime('yyyymmdd',dt_startDate.Date),FormatDateTime('yyyymmdd',dt_endDate.Date),stFoodArea,stFoodPermit)
  else if DBTYPE = 'PG' then stSql := PostGreSql.SelectTB_FOODEVENTD2DFoodState(FormatDateTime('yyyymmdd',dt_startDate.Date),FormatDateTime('yyyymmdd',dt_endDate.Date),stFoodArea,stFoodPermit)
  else if DBTYPE = 'MSSQL' then stSql := MSSql.SelectTB_FOODEVENTD2DFoodState(FormatDateTime('yyyymmdd',dt_startDate.Date),FormatDateTime('yyyymmdd',dt_endDate.Date),stFoodArea,stFoodPermit)
  else Exit;
  ...
  stSql := stSql + ' Order By a.FO_DATE,a.CO_COMPANYCODE,a.EM_CODE ';
  ...
  begin
    ...
    begin                          
      ...
      begin
        ...
        Cells[6,nRow] := FindField('FO_BREAK').AsString;
        Cells[7,nRow] := FindField('FO_LUNCH').AsString;
        Cells[8,nRow] := FindField('FO_DINNER').AsString;
        Cells[9,nRow] := FindField('FO_MIDNIGHT').AsString;
        Cells[10,nRow] := FindField('TOT').AsString;
        Cells[11,nRow] := FindField('FO_FOODAMT').AsString;
      ...
      Cells[4,nRow] := '';
      Cells[5,nRow] := '';
      Cells[6,nRow] := inttostr(nBreakSummary);
      Cells[7,nRow] := inttostr(nLunchSummary);
      Cells[8,nRow] := inttostr(nDinnerSummary);
      Cells[9,nRow] := inttostr(nMidNightSummary);
      Cells[10,nRow] := inttostr(nSummaryCount);
      Cells[11,nRow] := inttostr(nAmtSummary);
    end;

  end;
end;

procedure TfmD2DFdState.cmb_CompanyChange(Sender: TObject);
begin
...
procedure TfmD2DFdState.FormClose(Sender: TObject;
...
procedure TfmD2DFdState.FormCreate(Sender: TObject);
begin
...
procedure TfmD2DFdState.cmb_JijumChange(Sender: TObject);
begin
  LoadDepartCode(copy(JijumCodeList.Strings[cmb_Jijum.ItemIndex],1,3),copy(JijumCodeList.Strings[cmb_Jijum.ItemIndex],4,3),DepartCodeList,cmb_Depart);
  btn_SearchClick(btn_Search);
end;

procedure TfmD2DFdState.cmb_DepartChange(Sender: TObject);
begin
  btn_SearchClick(btn_Search);
end;

procedure TfmD2DFdState.cmb_PosiChange(Sender: TObject);
begin
  btn_SearchClick(btn_Search);
end;

procedure TfmD2DFdState.btn_ExcelClick(Sender: TObject);
var
  stRefFileName,stSaveFileName:String;
  stPrintRefPath : string;
  nExcelRowStart:integer;
...
function TfmD2DFdState.GetTitle: String;
var
  stTitle : string;
begin
...
procedure TfmD2DFdState.sg_reportResize(Sender: TObject);
var
  nTotWidth: integer;
  nWidth : integer;
  nLeft : integer;
...
procedure TfmD2DFdState.cmb_FoodAreaChange(Sender: TObject);
begin
  btn_SearchClick(btn_Search);

end;

procedure TfmD2DFdState.cmb_FoodPERMITChange(Sender: TObject);
begin
  btn_SearchClick(btn_Search);

end;

procedure TfmD2DFdState.FormShow(Sender: TObject);
var
  nIndex : integer;
begin
...
initialization
  RegisterClass(TfmD2DFdState);
Finalization
  UnRegisterClass(TfmD2DFdState);
