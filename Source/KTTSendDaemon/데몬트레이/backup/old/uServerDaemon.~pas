...
begin
  ...
  DeviceServerDeviceList:= TStringList.Create;  //DeviceClient 모드 ECU리스트
...
//    ConnectTimer.Enabled:= False;
  ...
  begin
    ...
    DeviceServerDeviceList.Clear;   //DeviceClient 모드 ECU리스트
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      StatusBar1.Panels[2].Text := 'FireDoorOpenList Add(' + inttostr(i) + ')';
      Application.ProcessMessages;
//      TEcuInfo(DeviceServerDeviceList.Objects[I]).DoorModeChange('1','1',True); //1번문 개방
//      Delay(200);
//      TEcuInfo(DeviceServerDeviceList.Objects[I]).DoorModeChange('2','1',True); //2번문 개방
//      Delay(200);
    end;
  end;
...
begin
  ...
  if Not CheckFTPDownLoadList then  //다운로드 리스트가 없으면 중단하고 빠져 나감.
  begin
    //FTPSendTimer.Enabled := True;
    bFTPSendProcess := False;
    Exit;
  ...
  begin
    bFTPSendProcess := False; //전송 실패 했으면 재전송
  end;
  //bFTPSendProcess := False;
  FTPSendTimer.Enabled := True;
//
...
begin
  ...
  if bFtpSendCheck then //bFtpSendCheck 는 # 게이지 오는 곳 에서만 셋팅 한다.
  begin
    bFtpSendCheck := False;
    Exit;
  end;
