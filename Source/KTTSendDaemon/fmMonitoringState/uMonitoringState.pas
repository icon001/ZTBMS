...
type
  TfmMonitoringState = class(TfmASubForm)
    AdvPanel1: TAdvPanel;
    AdvPanel2: TAdvPanel;
  ...
  ARow: Integer; Rect: TRect; State: TGridDrawState);
    ...
    procedure AdvStringGrid1KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure rg_EtcGubunClick(Sender: TObject);
  private
...
var
  fmMonitoringState: TfmMonitoringState;
...
procedure TfmMonitoringState.FormCreate(Sender: TObject);
begin
...
procedure TfmMonitoringState.CommandArrayCommandsTCommand0Execute(
...
procedure TfmMonitoringState.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  SearchTimer.Enabled := False;
  if G_bApplicationTerminate then Exit;
  self.FindSubForm('Main').FindCommand('MONITORINGSTATE').Params.Values['상태'] := 'FALSE';
  self.FindSubForm('Main').FindCommand('MONITORINGSTATE').Execute;
  Action := caFree;
  DisplayList.Clear;
  DisplayList.Free;
  SearchTimer.Free;
...
procedure TfmMonitoringState.FormShow(Sender: TObject);
begin

  self.FindSubForm('Main').FindCommand('MONITORINGSTATE').Params.Values['상태'] := 'TRUE';
  self.FindSubForm('Main').FindCommand('MONITORINGSTATE').Execute;
...
procedure TfmMonitoringState.StringGrideResize(Gride:TStringGrid);
var
...
procedure TfmMonitoringState.AdvStringGrid1Resize(Sender: TObject);
begin
  StringGrideResize(AdvStringGrid1);
end;

procedure TfmMonitoringState.StringGrideDrawCell(Sender: TObject; ACol,
  ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  DataInCell : string;
  nLeft,nTop : integer;
...
procedure TfmMonitoringState.FormActivate(Sender: TObject);
begin
  AdvStringGrid1.OnDrawCell:=StringGrideDrawCell;
end;

procedure TfmMonitoringState.btn_ClearClick(Sender: TObject);
var
  i:integer;
begin
  bClear := True;
...
procedure TfmMonitoringState.DisplayStringGrid(aGubun, aIP,aData,aTemp: string);
var
  stDisplay : string;
begin
...
procedure TfmMonitoringState.btn_FileSaveClick(Sender: TObject);
var
  aFileName: String;
  sDate: String;
  eDate: String;
...
procedure TfmMonitoringState.btn_startClick(Sender: TObject);
begin
  bStart := True;
...
procedure TfmMonitoringState.btn_stopClick(Sender: TObject);
begin
  bStart := False;
...
procedure TfmMonitoringState.CommandArrayCommandsTCommand1Execute(
  Command: TCommand; Params: TStringList);
begin
  Close;
...
procedure TfmMonitoringState.CloseForm;
begin
  Close;
end;

procedure TfmMonitoringState.SearchTimerTimer(Sender: TObject);
begin
  SearchTimer.Enabled := False;
...
procedure TfmMonitoringState.BatchDisplay(aData:string);
var
  stGubun : string;
  stIP : string;
  stData : string;
...
{  aCmd := FindCharCopy(aData,0,';');
  ...
  if rg_EtcGubun.ItemIndex = 1 then
  begin
    if UpperCase(stGubun) <> UpperCase(cmb_TxRx.Text) then Exit;
  end;
...
procedure TfmMonitoringState.mn_memoryClearClick(Sender: TObject);
var
  stNodeNo : string;
  stEcuID : string;
begin
...
//충대 프로젝트에서 ICU-200 도어상태 올라 오는것 처리용
procedure TfmMonitoringState.mn_DoorOpenClick(Sender: TObject);
var
  stNodeNo : string;
  stEcuID : string;
  stSendData : string;
...
procedure TfmMonitoringState.AdvStringGrid1KeyDown(Sender: TObject;
...
procedure TfmMonitoringState.rg_EtcGubunClick(Sender: TObject);
begin
  if rg_EtcGubun.ItemIndex = 0 then
  begin
    cmb_TxRx.Visible := False;
  end else if rg_EtcGubun.ItemIndex = 1 then
  begin
    cmb_TxRx.Visible := True;
  end;

end;

initialization
  RegisterClass(TfmMonitoringState);
Finalization
  UnRegisterClass(TfmMonitoringState);
