...
{  TDeviceLockState = Class(TComponent)
  ...
  TfmMain = class(TfmASubForm)
    ...
    procedure mn_fdmsResendClick(Sender: TObject);
  private
    //Component 모음
  ...
  private
    ...
    L_nEventdeleteEMPDay : integer;        // 사원 이력 데이터 보관일수
...
//    Function  GetUpdateAttendType(aATDate,aNowTime,aCompanyCode,aEMCode:string;dtYesterDay:TDatetime):string;
  ...
  public
    ...
    Function fdmsConnect(aMsg:Boolean=False):Boolean; //fdms Connect
    ...
    Function ShIntiFdmsUserAdd(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode:string):Boolean;
    Function ShIntiInsertFdmsMEMBER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode:string):Boolean;
    Function ShIntiUpdateFdmsMEMBER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode:string):Boolean;
    Function ShIntiCheckFdmsUser(aFDMS_ID:string):Boolean;
    Function ShIntiCheckFdmsVISITOR(aFDMS_ID:string):Boolean;
    Function ShIntiInsertTB_PATTERNDAY(aFDMS_ID:string):Boolean;
    Function ShIntiInsertTB_USERPATTERNLINK(aFDMS_ID:string):Boolean;
    Function ShIntiInsertTB_USERSICKLEAVE(aFDMS_ID:string):Boolean;
    Function ShIntiInsertTB_USER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode:string):Boolean;
    Function CheckIntiFdmsMEMBER(aFDMS_ID:string):Boolean;
  ...
  private
    ...
    Function LoadBuildingCode(aFirst:Boolean=True) : Boolean; //위치정보를 로드한다.
  ...
  private
    ...
    //AlarmEvent
    Procedure ArmAreaStatusChange(Sender: TObject;  aArmAreaCode,aArmRelayCode,aArmAreaName,aBuildingCode,aArmStatus,aVacancy:string ); //알람구역 상태 변경
    ...
    procedure RcvCardBufferFull(Sender: TObject;  aData: string;NodeNO : integer); // Card Buffer Full
...
begin
  ...
  if ini_fun.ReadString('DATABASE','MESSAGE','TRUE') = 'TRUE' then G_bDBErrorMessageShow := True
  else G_bDBErrorMessageShow := False;
...
begin
  if G_bApplicationTerminate then Exit;
  StatusBar1.Panels[2].Text := 'Timer1Timer';
  Visible:=False;
  Timer1.Enabled:=False;
end;
...
begin
  ...
  if Not TDataBaseConfig.GetObject.DBConnected then
  begin
    if G_bDBErrorMessageShow then showmessage('데이터베이스 접속 실패입니다. 데이터베이스 환경설정을 확인하여 주세요.');
    L_bClose := True;
    Application.Terminate;
    Exit;
  end;
...
StatusBar1.Panels[0].Text := 'GetDaemonConfig 로딩완료...';
  sendClientTimer.Interval := G_nSendClientTimerInterval;
...
StatusBar1.Panels[0].Text := 'LoadFireGroup 로딩중...';
    if G_bCardMemoryLoad then
    begin
      dmDBFunction.UpdateTB_DEVICECARDNO_AllMemory('N'); //대용량시 시간을 너무 많이 잡아 먹는다...ㅠ.ㅠ
      LoadDeviceCardNo; //카드권한 로딩
    end;
...
{  if dtSyncTime <= Now then
  ...
  Try
    ConnectTimer.Enabled := False;
    ConnectTimer.Interval := G_nConnectTimerInterval;
...
{$ENDIF}
      ...
      {        if (FindField('DE_RCVACK').IsNull) or (Trim(FindField('DE_RCVACK').asstring) = '')  then aSend:='N'
            ...
            //sleep(1);
            //MyProcessMessage;     //다시 안들어 오는 경우 발생
...
begin
  Try
    ...
    begin
      ...
      stResult:= Copy(ReceiveData,27+8+6,1);
    ...
    begin
      ...
      stResult:= Copy(ReceiveData,27+nCardNoLen+6,1);
...
*)
    ...
    if (Uppercase(chCmd)= 'L') and  (stResult = '1') then
    begin
      //카드 Buffer Full 이 난 상황이다.
      stDeviceID:=FillzeroNumber(NodeNo,3) + stECUID;

      nIndex := DeviceList.IndexOf(stDeviceID);
      if nIndex > -1 then
      begin
        TDevice(DeviceList.Objects[nIndex]).CardBufferFull := True;
      end;
    end else if (Uppercase(chCmd)= 'N') or (stResult = '2') then
    begin
      //카드 Buffer Full 이 해결 된 상황이다.
      stDeviceID:=FillzeroNumber(NodeNo,3) + stECUID;

      nIndex := DeviceList.IndexOf(stDeviceID);
      if nIndex > -1 then
      begin
        TDevice(DeviceList.Objects[nIndex]).CardBufferFull := False;
      end;
    end;
    // 자동으로 서버 타입에 맞춰서 카드 데이터 변환,2014-09-19
    StatusBar1.Panels[0].Text := inttostr(NodeNO) + stECUID + ';' + stCardNo + '수신';
        ...
        begin
          ...
          begin
           ...
           // 기기종류
            ...
            if FindField('AC_CARDFULL').AsString = '1' then aDevice.CardBufferFull := True
            else aDevice.CardBufferFull := False;
            ...
            aDevice.OnCardBufferFull    := RcvCardBufferFull;
            ...
            aDevice.OnSendData :=         CommNodeSend;
          ...
          begin
           ...
           // 기기종류
            ...
            if FindField('AC_CARDFULL').AsString = '1' then TDevice(DeviceList.Objects[nIndex]).CardBufferFull := True
            else TDevice(DeviceList.Objects[nIndex]).CardBufferFull := False;
...
//  ReceiveData := stringReplace(ReceiveData,#0,'0',[rfReplaceAll]);
  ...
  Try
    ...
    if cmd = 'A' then
    begin
      if G_nSpecialProgram = 10 then   //홍대 세종 캠퍼스 이면 방범상태 연동하자.   20190319
      begin
        nArmAreaIndex := ArmAreaCodeList.IndexOf(FillZeroNumber(NodeNO,3) + stEcuID + stZoneCode );
        if stStatus = 'ET' then
        begin
          if nArmAreaIndex > -1 then
          begin
            TRelayArmAreaCode(ArmAreaCodeList.Objects[nArmAreaIndex]).ArmStatus := stMode;
            TRelayArmAreaCode(ArmAreaCodeList.Objects[nArmAreaIndex]).Vacancy := stStatus;
          end;
        end else
        begin
          if nArmAreaIndex > -1 then
          begin
            TRelayArmAreaCode(ArmAreaCodeList.Objects[nArmAreaIndex]).Vacancy := ' ';
            TRelayArmAreaCode(ArmAreaCodeList.Objects[nArmAreaIndex]).ArmStatus := stMode;
          end;
        end;
      end;
...
{        stSql :=  'Insert into TB_ACCESSEVENT';
  ...
  //  begin
    ...
    *)
    ...
    begin
      StatusBar1.Panels[1].Text := 'bUseATDevice';
  ...
  Try
    ...
    begin
      ...
      begin
        ...
        begin
          if isdigit(FindField('CO_CONFIGVALUE').AsString) then G_nSendClientTimerInterval := strtoint(FindField('CO_CONFIGVALUE').AsString);
...
begin
  ...
  dtDaemonLogDeleteTime := IncDay(dtDaemonLogDeleteTime,1);
  ...
  Try
    ...
    begin
      ...
      begin
        ...
        else if FindField('CO_CONFIGCODE').AsString = 'EMP_DELETE' then L_nEventdeleteEMPDay := strtoint(FindField('CO_CONFIGVALUE').AsString)
...
begin
  ...
  if(L_nEventdeleteEMPDay > 0 ) then
  begin
    //시스템 로그 데이터 삭제
    stDate := FormatdateTime('yyyymmddhhnnss',Now - L_nEventdeleteEMPDay);
    stSql := ' Delete From TB_EMPHIS ';
    stSql := stSql + ' where EH_INSERTTIME <= ''' + stDate + ''' ';
    DataModule1.ProcessExecSQL(stSql);
  end;
...
begin
  ...
  Try
    ...
    begin
      //수신 완료 후 6초가 지났는데... 송신 성공이 안되었으면 송신중인 데이터 미송신으로 변경 후 다운로드 해제
      if Not L_bCardDownLoad then
      begin
        L_nCardSendCount := 0;
        L_nCardAckCount := 0;
...
begin
  ...
  Try
    ...
    //Application.ProcessMessages;
    CardAutoDownTimer.Interval := G_nCardAutoDownTimerInterval;
    CardAutoDownTimer.Enabled := False;
    CardDownLoadCheckTimer.Enabled := True;  ///1000초 동안 카드 다운로드에서 빠져 나오지 못하면 CardAutoDownTimer 재실행 히기기 위한 동작
    CardNoDownload(0,'00',False); //미전송 데이터 전송
...
function TfmMain.fdmsConnect(aMsg:Boolean=False): Boolean;
var
  conStr : string;
begin
  result := False;
  ...
  begin
    ...
    Except
      on E : EDatabaseError do
      begin
        if aMsg then showmessage(E.Message );
...
begin
  if G_bApplicationTerminate then Exit;
  Try
    DeviceInfoSendTimer.Interval := G_nG_nDeviceInfoSendTimerInterval;
    DeviceInfoSendTimer.Enabled := False;
...
begin
  ...
  showmessage('데이터 베이스 백업이 완료 되었습니다.');
...
begin
  ...
  begin
    //성현 통합 버젼 연동
    Result := ShIntiFdmsUserAdd(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode);
    Exit;
  end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (G_nSpecialProgram <> 1) and fdmsDeleteCardUse then
              begin
              ...
              if (G_nSpecialProgram <> 1) and fdmsAsciiToHex then
...
begin
  ...
  if Not L_bDaemonStart then Exit;
  StateCheckTimer.Enabled := False;
  StatusBar1.Panels[2].Text := '장비상태체크';
  for I:= 0 to DeviceList.Count -1 do
  begin
    if G_bApplicationTerminate then Exit;
    if TDevice(DeviceList.Objects[I]).Connected then
...
begin
  ...
  begin
    stSql := 'select * from Tuser ';
    stSql := stSql + ' Where id = ' + aFDMS_ID ;
  end else if fdmsType = '3' then //
  begin
    stSql := ' select * from TB_USER ';
    stSql := stSql + 'where userID = '''+ FillZeroNumber(strtoint(aFDMS_ID),16)+''' ';
  end else Exit;
...
begin
  result := False;
  stCardNo := aCA_CARDNO;
  if (G_nSpecialProgram = 1) then  //KT사옥이면 시리얼 카드번호는 4바이트 HEX값으로 넘겨 주고 사번인 경우는 HEX로 풀어서 넘겨주자.
  begin
    if( stCardNo[10] <> '*') then // isDigit(copy(stCardNo,1,11)) ) then
    begin
      stCardNo := AsciiLen2Hex(stCardNo,11);
    end else
    begin
      stCardNo := copy(stCardNo,1,8);
    end;
...
begin
  ...
  if (G_nSpecialProgram = 1) then  //KT사옥이면 시리얼 카드번호는 4바이트 HEX값으로 넘겨 주고 사번인 경우는 HEX로 풀어서 넘겨주자.
  begin
    if( stCardNo[10] <> '*') then //if( isDigit(copy(stCardNo,1,11)) ) then
    begin
      stCardNo := AsciiLen2Hex(stCardNo,11);
    end else
    begin
...
//  L_bFirstFire := False;
  ...
  stSql := CommonSql.UpdateTB_CONFIG('COMMON','DAEMON_VER','11');
  DataModule1.ProcessExecSQL(stSql);
...
begin
  ...
  if G_bApplicationTerminate then Exit;
  if Not L_bDaemonStart then Exit;
  FTPSendTimer.Interval := G_nFTPSendTimerInterval;
...
begin
  if G_bApplicationTerminate then Exit;
  StatusBar1.Panels[2].Text := 'PersonRelayTimerTimer';
...
begin
  ...
  if aDeviceID = '000' then
  begin
      StatusBar1.Panels[6].Text := 'FireRecovery';
      for i:= 0 to ComNodeList.Count - 1 do
      begin
        TCommnode(ComNodeList.Objects[i]).FireRecovery(True);
        if G_bFireRelayUse then
...
begin
  ...
  StatusBar1.Panels[2].Text := 'HolidaySendTimerTimer';
  Try
    HolidaySendTimer.Enabled := False;
    HolidaySendTimer.Interval := G_nHolidaySendTimerInterval;
...
begin
  StatusBar1.Panels[2].Text := 'LogDirectoryDeleteTimerTimer';
  if G_bApplicationTerminate then Exit;
  Try
...
begin
  if G_bApplicationTerminate then Exit;
//  StatusBar1.Panels[2].Text := 'ClientAckTimerTimer';
  SendClientData('ACK');
end;
...
begin
  CardAutoDownTimer.Enabled := False;  //카드 자동 다운로드 타이머 DB에서 메모리 로딩 타이머
  CardDownLoadCheckTimer.Enabled := False;  //1000초 동안 카드 다운로드에서 빠져 나오지 못하면 CardAutoDownTimer 재실행 히기기 위한 동작
  CardLoadTimer.Enabled := False;  ///카드 데이터 메모리 로딩 타이머(현재 사용 안함)
  ConnectTimer.Enabled := False;   //장비 접속 시도 타이머

  DataBaseBackupTimer.Enabled := False;  //데이터베이스 백업 타이머
  DBProcessTimer.Enabled := False; ///베모리에 있는 이벤트 DB에 Write하는 타이머
  DeviceLoadTimer.Enabled := False;   //변경된 장비 로딩
  DeviceInfoSearch.Enabled := False; //기기코드 조회

  fdmsRelyTimer.Enabled := False;    ///지문리더 연동 타이머
  FoodTimer.Enabled := False;   //식수코드 체크 타이머
  FTPSendTimer.Enabled := False;  //FTP 카드 데이터 다운로드 타이머
  FTPSendProcessChckTimer.Enabled := False; //FTP 전송중인지 체크하는 타이머

  HolidaySendTimer.Enabled := False;   //장비에 공휴일 데이터 전송

  LogDeleteTime.Enabled := False;   // 통신 로그 삭제 타이머

  RecvTimeCheck.Enabled := False;   //카드 데이터 전송후 전송 완료 유무 체크하는 타이머 (현재 사용 안함)
  RelayControlTimer.Enabled := False; //전남대병원 연동 프로그램 재기동 타이머

  SendTimer.Enabled := False;      //장비에 패킷 전송 타이머
  sendClientTimer.Enabled := False;  //모니터링 PC에 패킷 전송 타이머
  SentenceRelayTimer.Enabled := False; //전문 연동타이머(연동시에만 동작)
  SyncTimer.Enabled := False;     ///기기 시간 동기화 타이머
  StateCheckTimer.Enabled := False;   //장비상태체크 체크 타이머
  StateSendTimer.Enabled := False; //노드의 상태가 변경 되는 경우 클라이언트 PC에 상태 전송 타이머

  Timer1.Enabled := False;  //처음 화면 감추는 타이머

  ValidExpiredTimer.Enabled := False; //유효기간 지난 카드 삭제 타이머

  WebRelayTimer.Enabled := False;     // 김천 이노닉스 웹 연동 타이머, 김천에서만 동작
...
begin
  ...
  StatusBar1.Panels[2].Text := 'DeviceSortTimerTimer';
...
begin
  ...
  StatusBar1.Panels[2].Text := 'STRTimerTimer';
  STRTimer.Enabled := False;
...
begin
  ...
  //ShIntiFdmsUserAdd('2','2','2', 'test','01012341234', '부서', '직위','ABABABAC','1','001') ;
(*  ATEventList.SaveToFile(ExeFolder + '\..\log\ATEvent'+ FormatDateTIme('yyyymmdd',Now)+'.log');
  stTemp := '00120000000000000001400000000000268702625211a002001019T120057                       1이혜림';
          TdmAttendEvent(WorkTypeList.Objects[0]).Process_EmployeeATEvent(copy(stTemp,4,14),copy(stTemp,18,3),copy(stTemp,21,2),Trim(copy(stTemp,23,20)),
...
begin
  StatusBar1.Panels[2].Text := 'RestartTimerTimer';
...
begin
  //Exit;
  StatusBar1.Panels[2].Text := 'StateSendTimerTimer';
  if ClientSendList.Count > 0 then Exit; //전송건수가 1건 이상인 경우에는 나중에 전송하자.
  StateSendTimer.Interval := G_nStateCheckTimerInterval;
  Try
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'ClientDBSocketClearTimerTimer';
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'GroupPermitTimerTimer';
  Try
...
begin
  ...
  if aFirst then
  begin
    for i:= ArmAreaCodeList.count - 1 downto 0 do
    begin
      TRelayArmAreaCode(ArmAreaCodeList.Objects[i]).Free;
    end;
...
//    stSql := stSql + ' order by AC_NODENO,AC_ECUID,AL_ZONENUM ';
    ...
    begin
      ...
      begin
        ...
        stArmAreaNo := FillZeroNumber(nNodeNo,3) +  FindField('AC_ECUID').AsString + FillZeroNumber(FindField('AR_AREANO').AsInteger,2);
        nDeviceIndex:= ArmAreaCodeList.Indexof(stArmAreaNo);
        if nDeviceIndex > -1 then
        begin
          TRelayArmAreaCode(ArmAreaCodeList.Objects[nDeviceIndex]).ArmRelayCode :=  FindField('AR_RelayCode').asString;
          TRelayArmAreaCode(ArmAreaCodeList.Objects[nDeviceIndex]).ArmAreaName :=  FindField('AR_NAME').asString;
          TRelayArmAreaCode(ArmAreaCodeList.Objects[nDeviceIndex]).BuildingCode :=  FindField('LO_DONGCODE').AsString + FindField('LO_FLOORCODE').AsString + FindField('LO_AREACODE').AsString;

        end else
        begin
          aArmArea := TRelayArmAreaCode.Create(Self);
          aArmArea.ArmAreaCode :=  stArmAreaNo;
          aArmArea.ArmRelayCode :=  FindField('AR_RelayCode').asString;
          aArmArea.ArmAreaName :=  FindField('AR_NAME').asString;
          aArmArea.BuildingCode :=  FindField('LO_DONGCODE').AsString + FindField('LO_FLOORCODE').AsString + FindField('LO_AREACODE').AsString;
          aArmArea.Vacancy := ' ';
          aArmArea.OnArmAreaStatusChange := ArmAreaStatusChange;
          ArmAreaCodeList.addobject(stArmAreaNo,aArmArea);
        end;
...
begin
  inherited;
//  StatusBar1.Panels[2].Text := 'DBProcessTimerTimer(' + inttostr(AlarmEventList.Count) + '/' + inttostr(AccessEventList.Count) + '/' + inttostr(ATEventList.Count) + ')';
  ...
  (*      stEvent := stTimestr + FillZeroNumber(NodeNo,3) + stEcuID + cDoorNo + FillSpace(stCardNo,20) +
        ...
        begin
          stSql := CommonSql.InsertIntoTB_ACCEVENT(
                            ...
                            copy(stTemp,48,14), //FormatdateTime('yyyymmddHHMMSSzzz',Now),
    ...
    Try
      ...
      begin
        ...
        *)
        ...
        begin
//  StatusBar1.Panels[2].Text := inttostr(ATEventList.Count) + '\' + stTemp;
  ...
  End;
  //StatusBar1.Panels[2].Text := 'DBProcessTimerTimer_End(' + inttostr(AlarmEventList.Count) + '/' + inttostr(AccessEventList.Count) + ')';
end;
...
begin
  ...
  else stSql := stSql + ' AND a.DE_MEMLOAD <> ''Y'' ';
  stSql := stSql + ' order by a.de_permit,a.ca_cardno ';
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'LogServerTimerTimer';
  Try
    bLogServer := False;
...
begin
  ...
  FoodTimer.Interval := G_nFoodTimerInterval;
...
begin
  inherited;
  if G_bApplicationTerminate then Exit;
  StatusBar1.Panels[2].Text := 'DeviceInfoSearchTimer';
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'WebRelayTimerTimer';
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'CheckKTTSenderTimerTimer';
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'CardDeleteTimerTimer';
  Try
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'DaemonStateCheckTimerTimer';
  DaemonStateCheckTimer.Enabled := False;
  Try
    stSql := 'Select * from TB_CURRENTDAEMON ';
...
begin
  inherited;
  StatusBar1.Panels[2].Text := 'RelayControlTimerTimer';
  Try
    RelayControlTimer.Enabled  := False;
...
begin
  ...
  CardDownLoadCheckTimer.Enabled := False;
  StatusBar1.Panels[2].Text := 'CardDownLoadCheckTimer';
end;

function TfmMain.LoadBuildingCode(aFirst: Boolean): Boolean;
var
  stSql : String;
  aBuilding     : TRelayBuildingCode;
  TempAdoQuery : TADOQuery;
...
begin
  ...
  if (aFirst) then
  begin
    for i := 0 to BuildingCodeList.Count - 1 do     //구역경계 메모리 초기화
    begin
      TRelayBuildingCode(BuildingCodeList.Objects[i]).Free;
    end;
  ...
  Try
    ...
    stSql := stSql + ' where GROUP_CODE = ''' + GROUPCODE + '''';
    if ( Not aFirst ) then  stSql := stSql + ' AND LO_CHANGE = ''Y''';
    ...
    begin
      ...
      begin
        ...
        if nIndex > -1 then
        begin
          TRelayBuildingCode(BuildingCodeList.Objects[nIndex]).RelayBuildingCode := FindField('LO_RelayCODE').Asstring ;
        end else
        begin
          aBuilding:= TRelayBuildingCode.Create(Self);
          aBuilding.BuildingCode := stBuildingCode;
          aBuilding.RelayBuildingCode := FindField('LO_RelayCODE').Asstring;
          BuildingCodeList.AddObject(stBuildingCode,aBuilding);
        end;

        dmDBFunction.UpdateTB_LOCATION_Field_StringValue(FindField('LO_DONGCODE').Asstring,FindField('LO_FLOORCODE').Asstring,FindField('LO_AREACODE').Asstring,'LO_CHANGE','N');
        next;
      end;
    end;
  Finally
...
procedure TfmMain.ArmAreaStatusChange(Sender: TObject; aArmAreaCode,
  aArmRelayCode, aArmAreaName, aBuildingCode, aArmStatus,
  aVacancy: string);
var
  stNodeNo : string;
  stEcuID : string;
  stArmArea : string;
  stRelayBuilding : string;
  nIndex : integer;
begin
(*
  stNodeNo := copy(aArmAreaCode,1,3);
  stEcuID := copy(aArmAreaCode,4,2);
  stArmArea := copy(aArmAreaCode,6,2);
*)
(*  if Not dmRelayDB.HURelayDBConnected then
  begin
    if Not dmRelayDB.HURelayAdoConnect(DBTYPE,DBServer,DBPort,'ZMOS',DBUser,DBUserPw) then Exit;
  end;
*)
  if aArmRelayCode = '' then Exit; //연동 코드가 없으면 넣을 수 없다.
  
  stRelayBuilding := '';
  nIndex := BuildingCodeList.Indexof(aBuildingCode);
  if nIndex > -1 then
  begin
    stRelayBuilding := TRelayBuildingCode(BuildingCodeList.Objects[nIndex]).RelayBuildingCode;
  end;
  dmDBFunction.InsertIntoTB_HURELAY_Value(aArmRelayCode, aArmAreaName, stRelayBuilding,aArmStatus, aVacancy);
end;

procedure TfmMain.mn_UpdateClick(Sender: TObject);
begin
  inherited;
  if Not FileExists(ExeFolder + '\ZDaemon\SmartUpdate.exe') then
  begin
    showmessage('업데이트 도구가 설치되어 있지 않습니다.');
    Exit;
  end;

  My_RunDosCommand(ExeFolder + '\ZDaemon\SmartUpdate.exe',False,False);
  Close;

end;

(*
function TfmMain.InsertShFdmsUser(aFdmsKey, aEmCode, aFDMS_ID, aEM_NAME,
  aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO, aMode,
...
begin
  ...
  if (G_nSpecialProgram = 1) then  //KT사옥이면 시리얼 카드번호는 4바이트 HEX값으로 넘겨 주고 사번인 경우는 HEX로 풀어서 넘겨주자.
  begin
    if( isDigit(copy(stCardNo,1,11)) ) then
    begin
      stCardNo := AsciiLen2Hex(stCardNo,11);
    end else
    begin
  ...
  stCardNo:='3034'+stCardNo;

  ADOStoredProc1 := TADOStoredProc.Create(nil);
  with ADOStoredProc1 do
  begin
    Connection := DataModule1.ADOConnection;
    ProcedureName := 'SP_USER_CLIENT_SAVE';
    Parameters.Clear;
    Parameters.CreateParameter('RETURN_VALUE', ftInteger, pdReturnValue, 0, null);
    Parameters.CreateParameter('@savetype', ftInteger, pdInput, 0, 1);
    Parameters.CreateParameter('@userID', ftString, pdInput, 0, FillZeroStrNum(aFDMS_ID,16));
    Parameters.CreateParameter('@userName', ftString, pdInput, 0, aEM_NAME);
    Parameters.CreateParameter('@userNumber', ftString, pdInput, 0, aEmCode);
    Parameters.CreateParameter('@userTelNumber', ftString, pdInput, 0, aHandPhone);
    Parameters.CreateParameter('@departmentID', ftInteger, pdInput, 0, 1);
    Parameters.CreateParameter('@positionID', ftInteger, pdInput, 0, -1);
    Parameters.CreateParameter('@authMethod', ftInteger, pdInput, 0, 2816);
    Parameters.CreateParameter('@userInDay', ftDateTime, pdInput, 0, now);
    Parameters.CreateParameter('@userOutDay', ftDateTime, pdInput, 0, now + 100000);
    Parameters.CreateParameter('@userType', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@userStatus', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@userStatus', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@userCardNo1', ftTypedBinary, pdInput, 0, Hex2Ascii(stCardNo));
    Parameters.CreateParameter('@userCardNo2', ftTypedBinary, pdInput, 0, '');
    Parameters.CreateParameter('@userFpdata1', ftTypedBinary, pdInput, 0, '');
    Parameters.CreateParameter('@userFpdata2', ftTypedBinary, pdInput, 0, '');
    Parameters.CreateParameter('@userImage', ftTypedBinary, pdInput, 0, '');
    Parameters.CreateParameter('@loginAuthID', ftInteger, pdInput, 0, 3);
    Parameters.CreateParameter('@loginID', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@loginPW', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@authPW', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@uniquePW', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@userEmail', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@guardType', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@timetableUse', ftInteger, pdInput, 0, 4);
    Parameters.CreateParameter('@VaildTimeStart', ftDateTime, pdInput, 0, now);
    Parameters.CreateParameter('@VaildTimeEnd', ftDateTime, pdInput, 0, now+ 100000);
    Parameters.CreateParameter('@useInfo', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@infoname', ftString, pdInput, 0, 'kttelecop');
    Parameters.CreateParameter('@infoip', ftString, pdInput, 0, '000.000.000.000');
    Parameters.CreateParameter('@mobileip', ftString, pdInput, 0, '000.000.000.000');
    Parameters.CreateParameter('@videoport', ftInteger, pdInput, 0, 11142);
    Parameters.CreateParameter('@audioport', ftInteger, pdInput, 0, 11152);
    Parameters.CreateParameter('@isinfo', ftInteger, pdInput, 0, 0);
    Parameters.CreateParameter('@officephone', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@memo', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@userEngName', ftString, pdInput, 0, '');
    Parameters.CreateParameter('@damdang', ftInteger, pdInput, 0,0);
    Parameters.CreateParameter('@workexclude', ftInteger, pdInput, 0,1);
    Parameters.CreateParameter('@workstartda', ftDateTime, pdInput, 0,now);
    Parameters.CreateParameter('@workendday', ftDateTime, pdInput, 0,now+100000);
    Parameters.CreateParameter('@PhoneSort', ftInteger, pdInput, 0,1);
    Parameters.CreateParameter('@crc', ftInteger, pdInput, 0,0);
    Parameters.CreateParameter('@reguserid', ftString, pdInput, 0,'zeron');
    ExecProc;
    showmessage(inttostr(Parameters[0].Value));
...
function TfmMain.ShIntiFdmsUserAdd(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode: string): Boolean;
var
  stFdmsKey,stFdmsCardNo : string;
begin
  aFDMS_ID := FillZeroStrNum(aFDMS_ID,16);
  if (G_nSpecialProgram = 1) then  //KT사옥이면 시리얼 카드번호는 4바이트 HEX값으로 넘겨 주고 사번인 경우는 HEX로 풀어서 넘겨주자.
  begin
    if( aCA_CARDNO[10] <> '*') then //if( isDigit(copy(aCA_CARDNO,1,11)) ) then
    begin
      aCA_CARDNO := AsciiLen2Hex(aCA_CARDNO,11);
    end else
    begin
      aCA_CARDNO := copy(aCA_CARDNO,1,8);
    end;
  end;
  aCA_CARDNO:='3034'+aCA_CARDNO;
  if Not ShIntiCheckFdmsUser(aFDMS_ID) then
  begin
    if ShIntiCheckFdmsVISITOR(aFDMS_ID) then
    begin
      //연동 안됨
      Result := True;
      Exit;
    end;
    if CheckIntiFdmsMEMBER(aFDMS_ID) then
    begin
      //연동 안됨
      Result := True;
      Exit;
    end;
    Result := ShIntiInsertFdmsMEMBER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode);
  end else
  begin
    Result := ShIntiUpdateFdmsMEMBER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode);
  end;

end;

function TfmMain.ShIntiCheckFdmsUser(aFDMS_ID: string): Boolean;
var
  stSql : string;
begin
  result := False;
...
function TfmMain.ShIntiInsertFdmsMEMBER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode: string): Boolean;
begin
  ShIntiInsertTB_PATTERNDAY(aFDMS_ID);
  ShIntiInsertTB_USERPATTERNLINK(aFDMS_ID);
  ShIntiInsertTB_USERSICKLEAVE(aFDMS_ID);
  result := ShIntiInsertTB_USER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode);
end;

function TfmMain.ShIntiCheckFdmsVISITOR(aFDMS_ID: string): Boolean;
var
  stSql : string;
begin
  result := False;
  stSql := ' select * from TB_VISITOR ';
  stSql := stSql + 'where visitID = '''+ FillZeroNumber(strtoint(aFDMS_ID),16)+''' ';
...
function TfmMain.CheckIntiFdmsMEMBER(aFDMS_ID: string): Boolean;
var
  stSql : string;
begin
  result := False;
  stSql := ' select * from TB_MEMBER ';
  stSql := stSql + 'where memberID = '''+ FillZeroNumber(strtoint(aFDMS_ID),16)+''' ';
...
function TfmMain.ShIntiInsertTB_PATTERNDAY(aFDMS_ID: string): Boolean;
var
  stSql : string;
begin
  result := False;
  stSql := 'Insert Into TB_PATTERNDAY(userID,workPatternDay,workDayID,regdate,reguserid,moddate,moduserid) ';
  stSql := stSql + ' select ''' + aFDMS_ID + ''',workPatternDay,workDayID,GETDATE(),NULL,NULL,NULL from TB_PATTERNDAY_TEMP ';
  stSql := stSql + ' where userID = ''' + '0000000000000000' + '''';
  if SHF_ProcessExecSQL(stSql) > 0 then result := True;

end;

function TfmMain.ShIntiInsertTB_USERPATTERNLINK(aFDMS_ID: string): Boolean;
var
  stSql : string;
begin
  result := False;
  stSql := 'Insert Into TB_USERPATTERNLINK(userID,schedulePatternID,regdate,reguserid) ';
  stSql := stSql + ' Values(''' + aFDMS_ID + ''',1,GETDATE(),NULL )';
  if SHF_ProcessExecSQL(stSql) > 0 then result := True;

end;

function TfmMain.ShIntiInsertTB_USERSICKLEAVE(aFDMS_ID: string): Boolean;
var
  stSql : string;
begin
  result := False;
  stSql := 'Insert Into TB_USERSICKLEAVE(userID,sickleave,sickleaveSecond,regdate,reguserid) ';
  stSql := stSql + ' Values(''' + aFDMS_ID + ''',30,30*28800,GETDATE(),NULL )';
  if SHF_ProcessExecSQL(stSql) > 0 then result := True;
end;

function TfmMain.ShIntiInsertTB_USER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode: string): Boolean;
var
  stSql : string;
  TempQuery :TADOQuery;
  bArr : byDynamicArr;
begin
  result := False;
  stSql := 'Insert Into TB_USER(userID,userName,userNumber,userTelNumber,departmentID,positionID,authMethod, ';
  stSql := stSql + 'userInDay,userOutDay,VaildTimeStart,VaildTimeEnd,userType,userStatus,userCardNo1,userCardNo2,';
  stSql := stSql + 'userFpdata1,userFpdata2,loginAuthID,uniquePW,guardType,';
  stSql := stSql + 'timeTableUse,multiAuth,infoip,mobileip,videoport,audioport,isinfo,infoname,crc,';
  stSql := stSql + 'useInfo,saveAccess, damdang,workexclude,workstartday,workendday,PhoneSort,regdate,reguserid) ';
  stSql := stSql + ' Values(''' + aFDMS_ID + ''',''' + aEM_NAME + ''',''' + aEmCode + ''',''' + aHandPhone + ''',';
  stSql := stSql + '1,-1,2816,GETDATE(),''2037-12-31 23:59:59'',GETDATE(),''2037-12-31 23:59:59'',0,';
  stSql := stSql + '0,:Card_No1,:Card_No2,:Fpdata1,:Fpdata2,3,0,0,4,0,''000.000.000.000'',''000.000.000.000'',11142,11152,0,';
  stSql := stSql + '''kttelecop'',0,0,''Y'',0,1,GETDATE(),''2037-12-31 23:59:59'',1,GETDATE(),''ZERON'')';
  Try
    CoInitialize(nil);
    TempQuery := TADOQuery.Create(nil);
    TempQuery.Connection := ADOfdmsConnection;
    ...
    begin
      ...
      SQL.Text:= stSql;
      try
        bArr := Hex2ByteArray(aCA_CARDNO);
        Parameters.ParamByName('Card_No1').Value := bArr;//Hex2Ascii(aCA_CARDNO);
        bArr := Hex2ByteArray('00');
        Parameters.ParamByName('Card_No2').Value := bArr;
        Parameters.ParamByName('Fpdata1').Value := bArr;
        Parameters.ParamByName('Fpdata2').Value := bArr;
        ExecSQL;
        result := True;
      except
      ON E: Exception do
...
function TfmMain.ShIntiUpdateFdmsMEMBER(aEmCode,aFDMS_ID, aEM_NAME,aHandPhone, aDEPART_NAME, aPO_NAME, aCA_CARDNO,aMode,aJijumCode: string): Boolean;
var
  stSql : string;
  TempQuery :TADOQuery;
  bArr : byDynamicArr;
begin
  result := False;
  stSql := 'Update TB_USER set userName = ''' + aEM_NAME + ''',';
  stSql := stSql + 'userNumber = ''' + aEmCode + ''',';
  stSql := stSql + 'userTelNumber= ''' + aHandPhone + ''',';
  stSql := stSql + 'userCardNo1=:Card_No1 ';
  stSql := stSql + ' where userID =''' + aFDMS_ID + ''' ';
  Try
    CoInitialize(nil);
    TempQuery := TADOQuery.Create(nil);
    TempQuery.Connection := ADOfdmsConnection;
...
procedure TfmMain.mn_fdmsResendClick(Sender: TObject);
var
  stSql : string;
begin
  stSql := ' Update TB_EMPHIS set SEND_STATUS = ''N'' ';

  DataModule1.ProcessExecSQL(stSql);
end;

procedure TfmMain.RcvCardBufferFull(Sender: TObject; aData: string;
  NodeNO: integer);
var
  stAlarmData : String;
  stEcuID : string;
begin
  if Not G_bCardFullUse then Exit; //옵션이 설정되어 있지 않다면 이벤트 발생시키지 말자.
  stEcuID := TDevice(Sender).ECUID;
  if(aData='1') then
  begin
    stAlarmData := '058 K10000000' + stEcuID + 'AJn'+ FormatDateTime('yyyymmddhhnnss',now) +'D' + '0' +'0000n' + 'CF' + '*************B8';
    DeviceRcvAlarmData(Sender,'A',stAlarmData,NodeNo,'001');
  end else
  begin
    stAlarmData := '058 K10000000' + stEcuID + 'AJn'+ FormatDateTime('yyyymmddhhnnss',now) +'D' + '0' +'0000n' + 'C0' + '*************B8';
    DeviceRcvAlarmData(Sender,'A',stAlarmData,NodeNo,'001');
  end;
