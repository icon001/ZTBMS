...
uses
  Windows,SysUtils, Classes,ActiveX,ADODB,DB;
...
begin
  ...
  Try
    CoInitialize(nil);
    TempAdoQuery := TADOQuery.Create(nil);
    TempAdoQuery.Connection := dmZeronEventDataBase.ADOEventConnection;
    with TempAdoQuery do
    begin
      Close;
      SQL.Clear;
...
begin
  ...
  result := dmZeronEventDataBase.ProcessEventExecSQL(stSql);
end;
...
begin
  result := False;
  if Not dmZeronEventDataBase.DBSessionCheck then
  begin
    TZeronEventDataBaseConfig.GetObject.DBConnected := False;
    TZeronEventDataBaseConfig.GetObject.DataBaseConnect;
    if TZeronEventDataBaseConfig.GetObject.DBConnected then Exit;
  end;
