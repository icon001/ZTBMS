...
type
  TfmNonATReport = class(TfmASubForm)
    GroupBox1: TGroupBox;
    Label1: TLabel;
    Label2: TLabel;
...
procedure TfmNonATReport.btn_CloseClick(Sender: TObject);
begin
  Close;

end;

procedure TfmNonATReport.FormActivate(Sender: TObject);
begin
...
procedure TfmNonATReport.LoadCompanyCode(cmb_Box: TComboBox);
var
  stSql :string;
begin
  cmb_Box.Clear;
...
procedure TfmNonATReport.LoadDepartCode(aCompanyCode: string;
  cmb_Box: TComboBox);
var
  stSql :string;
begin
...
procedure TfmNonATReport.FormCreate(Sender: TObject);
begin
...
procedure TfmNonATReport.FormClose(Sender: TObject;
...
procedure TfmNonATReport.cmb_CompanyChange(Sender: TObject);
begin
  LoadDepartCode(CompanyCodeList.Strings[cmb_Company.ItemIndex],cmb_Depart);

end;

procedure TfmNonATReport.btn_FileSaveClick(Sender: TObject);
var
  aFileName: String;
begin
...
procedure TfmNonATReport.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
 if AnsiCompareText(ParName, 'aDAY') = 0 then
    ParValue := '검색일 : '+ FormatdateTime('yyyy-mm-dd',dt_startDate.Date) ;

end;

procedure TfmNonATReport.btn_PrintClick(Sender: TObject);
begin
...
procedure TfmNonATReport.btn_SearchClick(Sender: TObject);
var
  stSql : string;
  i : integer;
  nRow : integer;
begin
  ...
  stSql := 'select ''' + formatDateTime('yyyy-mm-dd',dt_startDate.Date) + ''' as CurDate,';
  stSql := stSql + 'a.CA_CARDNO,a.GROUP_CODE,a.CO_COMPANYCODE,a.EM_CODE, ';
  stSql := stSql + '  b.CO_DEPARTCODE,b.EM_NAME,c.CO_NAME as COMPANY_NAME,d.CO_NAME as DEPART_NAME,   b.PO_POSICODE,e.PO_NAME  ';
  stSql := stSql + 'from  ';
  stSql := stSql + '(select a.CA_CARDNO,a.GROUP_CODE,a.EM_CODE,a.CO_COMPANYCODE ';
  stSql := stSql + 'from TB_CARD a  ';
  stSql := stSql + 'Where Not Exists  ';
  stSql := stSql + '(select * from TB_ATEVENT b  ';
  stSql := stSql + 'where b.AT_DATE = ''' + formatDateTime('yyyymmdd',dt_startDate.Date) + '''  ';
  stSql := stSql + 'AND a.GROUP_CODE = b.GROUP_CODE  ';
  stSql := stSql + 'AND a.CA_CARDNO = b.CA_CARDNO) ) a   ';
  ...
  stSql := stSql + 'Order by a.CO_COMPANYCODE,b.CO_DEPARTCODE,a.EM_CODE   ';
  ...
  begin
    ...
    begin
      with sg_Report do
      begin
        Cells[0,nRow] := FindField('CurDate').AsString;
...
initialization
  RegisterClass(TfmNonATReport);
Finalization
  UnRegisterClass(TfmNonATReport);
