...
procedure TfmPerFdState.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
 if AnsiCompareText(ParName, 'aDAY') = 0 then
    ParValue := '검색월 : '+ FormatdateTime('yyyy-MM',dt_Date.Date)  ;
...
begin
  ...
  stSql := stSql + ' FROM (select a.GROUP_CODE,a.CA_CARDNO,  ';
  ...
  stSql := stSql + ' SUM(a.FO_FOODAMT) as FO_FOODAMT ';
  stSql := stSql + ' From   ';
  stSql := stSql + ' ( select GROUP_CODE,FO_DATE,CA_CARDNO,FO_FOODAMT,  ';
  stSql := stSql + ' CASE  ';
  ...
  stSql := stSql + ' AND FO_DATE BETWEEN ''' + FormatDateTime('yyyymm',dt_Date.Date) + '00'' ';
  stSql := stSql + ' AND ''' + FormatDateTime('yyyymm',dt_Date.Date) + '32'' ';
  stSql := stSql + ' ) a   ';
  stSql := stSql + ' GROUP BY a.GROUP_CODE,a.CA_CARDNO ) a  ';
  ...
  stSql := stSql + ' Order By b.CO_COMPANYCODE,b.EM_CODE ';
...
procedure TfmPerFdState.LoadCompanyCode(cmb_Box: TComboBox);
var
  stSql :string;
begin
  cmb_Box.Clear;
...
procedure TfmPerFdState.LoadDepartCode(aCompanyCode: string;
  cmb_Box: TComboBox);
var
  stSql :string;
begin
...
procedure TfmPerFdState.FormActivate(Sender: TObject);
begin
  dt_Date.Date := Now;
