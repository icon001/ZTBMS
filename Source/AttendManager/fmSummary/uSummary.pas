...
type
  ...
  private
    ...
    L_nATType : integer;           //0:고정근무타입,1:사원별근무타입,2:교대근무타입
...
procedure TfmSummary.btn_CloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfmSummary.FormActivate(Sender: TObject);
var
  nIndex : integer;
begin
...
procedure TfmSummary.cmb_CompanyChange(Sender: TObject);
begin
...
procedure TfmSummary.cmb_JijumChange(Sender: TObject);
begin
  LoadDepartCode(copy(JijumCodeList.Strings[cmb_Jijum.ItemIndex],1,3),copy(JijumCodeList.Strings[cmb_Jijum.ItemIndex],4,3),DepartCodeList,cmb_Depart);

end;

procedure TfmSummary.btn_SummaryClick(Sender: TObject);
var
  lastDay: TDate;
  preLastDay : TDate;
  stLastDay : string;
...

{
  ...
  begin
    ...
    begin
      ...
      if L_nMonthLastDay < strtoint(stpreLastDay) then //월 분리일자가 지난달 마지막 일자 보다 작으면
      begin
        for i := (strtoint(stpreLastDay) + 1) to L_nMonthLastDay do
        begin
          stDate := stPreMonth + FillZeroNumber(i,2);
      ...
      if L_nMonthLastDay < strtoint(stLastDay) then stLastDay := inttostr(L_nMonthLastDay);//월 분리일자가 지난달 마지막 일자 보다 작으면
      for i := 1 to strtoint(stLastDay) do
      begin
      ...
      MonthSummary(stMonth,stPreMonth,
                   FindField('CO_COMPANYCODE').AsString,
                   FindField('EM_CODE').AsString,
                   FindField('PA_GUBUN').AsString,
                   FindField('PA_AMT').AsString );
...
function TfmSummary.CheckHolidy(aYear, aMonth, aDay: string): Boolean;
var
  stSql : string;
begin
  result := False;
...
function TfmSummary.LoadConfig: Boolean;
var
  stSql : string;
...
(*
function TfmSummary.DaySummary(aDate, aWeekCode, aCompanyCode,
  aEmCode,aPayGubun,aPayAmt,aWorkStartTime,aWorkEndTime,aJijumCode,aDepartCode,aEmName: string): Boolean;
var
  stSql : string;
...
function TfmSummary.VacationCheck(aDate,aCompanyCode, aEmCode: string; var aPayVacation,
...
function TfmSummary.InsertTB_ATDAYSUMMARY(aDate, aCompanyCode, aEmCode,
...
function TfmSummary.DiffMinute(aLargeTime, aSmallTime: string;aCheck:integer): integer;
var
  stHH,stMM : string;
  nHH,nMM : integer;
  bYesterDay1 : Boolean;
...
function TfmSummary.GetPerAmt(aTimeAmt,aMinuate,aPer:integer;bFixedAmt:Boolean): integer;
begin
...
function TfmSummary.MonthSummary(aMonth,aPreMonth,aCompanyCode, aEmCode,
  aPayGubun, aPayAmt: string): Boolean;
var
  stStartDate,stEndDate:string;
//  stSql : string;
begin
  if Not isDigit(aPayGubun) then aPayGubun := '0';
...
function TfmSummary.GetWorkTime(var aWorkStartTime,
...
function TfmSummary.MonthHolidaySummary(aMonth,aStartDate, aEndDate, aCompanyCode,
...
function TfmSummary.MonthWeekSummary(aMonth,aStartDate, aEndDate, aCompanyCode,
  aEmCode, aPayGubun, aPayAmt: string): Boolean;
var
  stSql : string;      //공휴일
  nTotCount : integer; //총 일수
...
function TfmSummary.DeleteTB_AtMonthSummary(aMonth, aCompanyCode,
...
function TfmSummary.DeleteTB_AtDAYSUMMARY(aDate, aCompanyCode,
  aEmCode: string): Boolean;
var
  stSql : string;
begin
...
function TfmSummary.InsertTB_ATMonthSummary(aMonth, aCompanyCode, aEmCode,
...
function TfmSummary.UpdateTB_ATMonthSummary(aMonth, aCompanyCode, aEmCode,
...
function TfmSummary.GetPersonType(aCompanyCode,aEmCode,aWeekCode: string;aTimeAmt:integer; var aInfromAdd,
...
procedure TfmSummary.ErrorCount(Sender: TObject; EventType,
  EventData: string);
begin
