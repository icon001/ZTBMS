...
uses
  ...
  Gauges,ADODB, uSubForm, CommandArray,ActiveX, DB;
...
type
  ...
  private
    ...
    function CreatePosiCode(aCompanyCode:string):Boolean;
...
begin
  ...
  GridInit(sg_CompanyCode,2);
  GridInit(sg_jijumCode,2);
  GridInit(sg_DepartCode,2);
  GridInit(sg_Posi,2);
end;
...
begin
  ...
  begin
    ...
    cmb_CompanyCode1.ItemIndex := cmb_Company1.ItemIndex;
  ...
  begin
    ...
    cmb_CompanyCode2.ItemIndex := cmb_Company2.ItemIndex;
    cmb_jijumCode2.ItemIndex := cmb_jijum2.ItemIndex;
  ...
  begin
    ...
    cmb_CompanyCode3.ItemIndex := cmb_Company3.ItemIndex;
...
begin
  ...
  begin
    ...
    begin
      ...
      if bResult then
      begin
        if (Application.MessageBox(PChar('직위코드를 기본값으로 생성하시겠습니까?'),'직위',MB_OKCANCEL) = IDOK)  then 
        begin
          CreatePosiCode(ed_Companycode.Text);
        end;
      end;
    end
...
function TfmCompanyCode.CreatePosiCode(aCompanyCode: string): Boolean;
var
  stSql : string;
begin
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'001','회장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'002','부회장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'003','사장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'004','부사장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'005','전무');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'006','상무');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'007','감사');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'008','고문');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'009','자문');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'010','이사');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'011','부장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'012','차장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'013','과장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'014','대리');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'015','주임');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'016','사원');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'017','Part장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'018','Team장');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'019','연구원');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'020','주임연구원');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'021','선임연구원');
  DataModule1.ProcessExecSQL(stSql);
  stSql := CommonSql.InsertIntoTB_Posi(aCompanyCode,'022','수석연구원');
  DataModule1.ProcessExecSQL(stSql);
...
begin
  stCompanyCode := '';
  if cmb_CompanyCode1.ItemIndex > 0 then stCompanyCode := copy(cmb_CompanyCode1.Text,1,3);
...
begin
  stCompanyCode := '';
  stJijumCode := '';
  if cmb_CompanyCode2.ItemIndex > 0 then stCompanyCode := copy(cmb_CompanyCode2.Text,1,3);
  if cmb_jijumCode2.ItemIndex > 0 then stJijumCode := copy(cmb_jijumCode2.Text,1,3);
...
begin
  stCompanyCode := '';
  if cmb_CompanyCode3.ItemIndex > 0 then stCompanyCode := copy(cmb_CompanyCode3.Text,1,3);
...
procedure TfmCompanyCode.cmb_jijum2Change(Sender: TObject);
begin
  ShowDepartCode('');
end;
