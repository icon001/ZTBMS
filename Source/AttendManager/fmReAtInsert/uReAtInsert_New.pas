...
//  dmAttendEvent.OnSendEvent := ATSendClientData;
end;
...
begin
  ...
  if ed_emCode.Text <> '' then stSql := stSql + ' and em_code = ''' + ed_emCode.Text + ''' ';
  DataModule1.ProcessExecSQL(stSql);
  stSql := 'Delete From TB_ATLISTEVENT Where AC_DATE Between ''' + stFromDate + ''' and ''' + stToDate + ''' ';
  if ed_emCode.Text <> '' then stSql := stSql + ' and em_code = ''' + ed_emCode.Text + ''' ';
  DataModule1.ProcessExecSQL(stSql);

  if DBTYPE = 'MDB' then stSql := MDBSql.SelectTB_ACCESSEVENTJOINATDEVICE(stFromDate,stToDate,ed_emCode.Text)
  else if DBTYPE = 'MSSQL' then stSql := MSSql.SelectTB_ACCESSEVENTJOINATDEVICE(stFromDate,stToDate,ed_emCode.Text)
  else if DBTYPE = 'PG' then stSql := PostGresql.SelectTB_ACCESSEVENTJOINATDEVICE(stFromDate,stToDate,ed_emCode.Text)
  else Exit;
  Try
    btn_AtReInsert.Enabled := False;
    CoInitialize(nil);
    ...
    begin
      ...
      begin
        ...
        begin
          if (FindField('CO_COMPANYCODE').Asstring <> '') and (FindField('CO_COMPANYCODE').Asstring <> '000') then stCompanyCode := FindField('CO_COMPANYCODE').Asstring;
          if (FindField('EM_CODE').Asstring <> '') and (FindField('EM_CODE').Asstring <> '000') then stEMCode := FindField('EM_CODE').Asstring;
          if (FindField('CO_JIJUMCODE').Asstring <> '') and (FindField('CO_JIJUMCODE').Asstring <> '000') then stJijumCode := FindField('CO_JIJUMCODE').Asstring;
          if (FindField('CO_DEPARTCODE').Asstring <> '') and (FindField('CO_DEPARTCODE').Asstring <> '000') then stDepartCode := FindField('CO_DEPARTCODE').Asstring;
          if FindField('EM_NAME').Asstring <> '' then stEmName := FindField('EM_NAME').Asstring;

//TEST
//showmessage('1 - ' + stEMCode);
        ...
        begin
          if (FindField('CO_COMPANYCODE').Asstring <> '') and
             (FindField('EM_CODE').Asstring <> '') then
          begin
            stCompanyCode := '';
            stEMCode := '';
            stJijumCode := '';
            ...
            stEmName := '';
            stAWCode := '';
            if FindField('CO_COMPANYCODE').Asstring <> '' then stCompanyCode := FindField('CO_COMPANYCODE').Asstring;
            if FindField('EM_CODE').Asstring <> '' then stEMCode := FindField('EM_CODE').Asstring;
            if FindField('CO_JIJUMCODE').Asstring <> '' then stJijumCode := FindField('CO_JIJUMCODE').Asstring;
            if FindField('CO_DEPARTCODE').Asstring <> '' then stDepartCode := FindField('CO_DEPARTCODE').Asstring;
            if FindField('EM_NAME').Asstring <> '' then stEmName := FindField('EM_NAME').Asstring;
