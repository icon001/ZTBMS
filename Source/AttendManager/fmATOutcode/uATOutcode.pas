...
type
  ...
  private
    ...
    function GetAutoATOutCode:string;
  public
    { Public declarations }
  end;
...
begin
  ...
  if chk_AutoCode.Checked then ed_Code.Text := GetAutoATOutCode;
  ed_Code.SetFocus;
end;
...
procedure TfmATOutcode.chk_AutoCodeClick(Sender: TObject);
begin
  if chk_AutoCode.Checked then ed_Code.Text := GetAutoATOutCode;

end;

function TfmATOutcode.GetAutoATOutCode: string;
var
  stSql : string;
  nSeq : integer;
begin
  nSeq := 11;
  stSql := CommonSql.GetMaxATOUTCode;
  with ADOQuery do
  begin
    Close;
    Sql.Clear;
    ...
    begin
      if IsDigit(FindField('AT_OUTCODE').AsString) then
      begin
        nSeq := strtoint(FindField('AT_OUTCODE').AsString) + 1;
...
procedure TfmATOutcode.ed_codeChange(Sender: TObject);
begin
