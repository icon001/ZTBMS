...
begin
  ...
  if cmb_Box <> nil then
  begin
    if isDigit(stTemp[1]) then cmb_Box.ItemIndex := strtoint(stTemp[1]) // 카드리더 사용여부
  ...
  if cmb_Box <> nil then
  begin
    if isDigit(stTemp[3]) then cmb_Box.ItemIndex := strtoint(stTemp[3]) //출입문 번호
    else cmb_Box.ItemIndex := 0;
    cmb_Box.Color := clYellow;
  end;
  ...
  if cmb_Box <> nil then
  begin
    if isDigit(stTemp[2]) then cmb_Box.ItemIndex := strtoint(stTemp[2]) // 문 위치
    else cmb_Box.ItemIndex := 0;
    cmb_Box.Color := clYellow;
  end;
  ...
  if cmb_Box <> nil then
  begin
    if isDigit(stTemp[22]) then cmb_Box.ItemIndex := strtoint(stTemp[22]) //빌딩 위치
    else cmb_Box.ItemIndex := 0;
    cmb_Box.Color := clYellow;
  end;
...
begin
  ...
  stDeviceID := FillZeroNumber(strtoint(aNodeNo),3) + aEcuID;
  stSendData := 'Ct00' + inttostr(cmb_ReaderType.ItemIndex - 1);                               //카드리더 타입 등록
