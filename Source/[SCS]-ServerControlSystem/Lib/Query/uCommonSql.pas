...
type
  ...
  public
    ...
    Function InsertIntoTB_ACCEVENT(aDate,aTime,aNodeNo,aEcuID,aDoorNo,aCardNo,aReaderNo,aButton,aDoorPosi,
             aInputType,aDoorMode,aPermitMode,aPermitCode,aInsertDate,aSCSHis,aAuthType:string):string;
    Function InsertIntoTB_AUTHTYPE(aCode,aCodeName:string):string;
    ...
    Function UpdateTB_ACCESSEVENTSCSHIS(aDate,aTime,aNodeNo,aEcuID,aDoorNo,aCardNo,aScsHis:string):string;
    Function UpdateTB_ACCESSEVENTAUTHTYPE(aDate,aTime,aNodeNo,aEcuID,aDoorNo,aCardNo,aAuthType:string):string;
    Function UpdateTB_ACCESSEVENTEmployeeInfo(aDate,aTime,aNodeNo,aEcuID,aDoorNo,aCardNo,
             aEmCode,aCompanyCode,aJijumCode,aDepartCode,aPosiCode,aEmName,aSCSResult,aAuthType:string ):string;
    //삭제
...
function TCommonSql.InsertIntoTB_ACCEVENT(aDate, aTime, aNodeNo, aEcuID,
  ...
  aPermitMode, aPermitCode, aInsertDate,aSCSHis,aAuthType: string): string;
var
  stSql : string;
begin
    ...
    stSql := stSql + 'AC_DOORMODE,AC_PERMITMODE,PE_PERMITCODE,AC_INSERTDATE,AC_SCSHIS,AC_AUTHTYPE)';
    stSql := stSql + ' Values(';
    stSql := stSql + '''' + GROUPCODE + ''',';
    stSql := stSql + '''' + aDate + ''',';
    stSql := stSql + '''' + aTime + ''',';
    ...
    stSql := stSql + '''' + aSCSHis + ''',';
    stSql := stSql + '''' + aAuthType + ''')';

    result := stSql;
end;

function TCommonSql.UpdateTB_ACCESSEVENTSCSHIS(aDate, aTime, aNodeNo,
  aEcuID, aDoorNo, aCardNo, aScsHis: string): string;
var
  stSql : string;
begin

  stSql := ' Update TB_ACCESSEVENT set AC_SCSHIS = ''' + aScsHis + ''' ';
  stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND AC_DATE = ''' + aDate + ''' ';
...
function TCommonSql.InsertIntoTB_AUTHTYPE(aCode,
  aCodeName: string): string;
var
  stSql : string;
begin
    stSql :=  'Insert into TB_AUTHTYPE';
    stSql := stSql + '(GROUP_CODE,AT_CODE,AT_NAME)';
    stSql := stSql + ' Values(';
    stSql := stSql + '''' + GROUPCODE + ''',';
    stSql := stSql + '''' + aCode + ''',';
...
function TCommonSql.UpdateTB_ACCESSEVENTAUTHTYPE(aDate, aTime, aNodeNo,
  aEcuID, aDoorNo, aCardNo, aAuthType: string): string;
var
  stSql : string;
begin

  stSql := ' Update TB_ACCESSEVENT set AC_AUTHTYPE = ''' + aAuthType + ''' ';
  stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND AC_DATE = ''' + aDate + ''' ';
  stSql := stSql + ' AND AC_TIME = ''' + aTime + ''' ';
  stSql := stSql + ' AND AC_NODENO = ' + aNodeNo + ' ';
...
function TCommonSql.UpdateTB_ACCESSEVENTEmployeeInfo(aDate, aTime, aNodeNo,
  aEcuID, aDoorNo, aCardNo, aEmCode, aCompanyCode, aJijumCode, aDepartCode,
  aPosiCode, aEmName, aSCSResult, aAuthType: string): string;
var
  stSql : string;
begin
...
//  stSql := stSql + ' PO_POSICODE = ''' + aPosiCode + ''', ';
  ...
  stSql := stSql + ' AC_SCSHIS = ''' + aSCSResult + ''', ';
  stSql := stSql + ' AC_AUTHTYPE = ''' + aAuthType + ''' ';
  stSql := stSql + ' Where GROUP_CODE = ''' + GROUPCODE + ''' ';
  stSql := stSql + ' AND AC_DATE = ''' + aDate + ''' ';
  stSql := stSql + ' AND AC_TIME = ''' + aTime + ''' ';
  stSql := stSql + ' AND AC_NODENO = ' + aNodeNo + ' ';
